 ...
 *
 * The existing WorkoutLog format is intentionally kept backward compatible;
 * this layer turns its parallel arrays into explicit set records and preserves
 * zero-rep attempts as real observations.
 */
...
export interface CanonicalSession {
  ...
  durationSec: number;
  readiness: Readiness;
  logs: CanonicalWorkoutLog[];
  totalReps: number;
  emomReps: number;
  bestSkillSeconds: number;
  sessionNote?: string;
...
function clampRir(value: unknown): number | undefined {
  if (!finite(value)) return undefined;
  return Math.max(0, Math.min(3, value));
}

function clampFatigue(value: unknown): number | undefined {
  if (!finite(value)) return undefined;
  return Math.max(1, Math.min(5, value));
}

function cleanArray(value: unknown): number[] {
  return Array.isArray(value) ? value.filter(nonNegative) : [];
}

/** Convert the legacy parallel result arrays into explicit records. */
export function toSetRecords(log: WorkoutLog): SetRecord[] {
  const reps = cleanArray(log.result.reps);
  const seconds = cleanArray(log.result.seconds);
  const emom = cleanArray(log.result.emom);
  ...
  if (log.kind === "EMOM") {
    return emom.map((value, index) => ({
      index,
      reps: value,
      rir: clampRir(log.result.rir),
      fatigue: clampFatigue(log.result.fatigue),
      band: log.result.band,
      failed: value === 0,
    }));
  }
  ...
  return Array.from({length: count}, (_, index) => {
    ...
    record.failed = (record.reps === 0 || record.seconds === 0) && log.status !== "skipped";
    return record;
  });
}

/**
 * Normalizes a persisted log without changing its historical meaning.
 * In particular, 0 reps is retained; only an explicit skipped status means skipped.
 */
export function canonicalizeWorkoutLog(log: WorkoutLog): CanonicalWorkoutLog {
  ...
  return {
    ...
    date: finite(log.date) ? log.date : 0,
    day: log.day,
    exerciseId: String(log.exerciseId),
    exerciseName: String(log.exerciseName),
    variantId: String(log.variantId || log.exerciseId),
    variantName: log.variantName,
    kind: log.kind,
    status: skipped ? "skipped" : log.status,
    skipped,
    modified: log.status === "modified" || Boolean(log.modification),
    modification: log.modification,
    sets: toSetRecords(log),
    band: log.result.band,
    note: log.result.note,
  };
}
...
export function canonicalizeSession(session: SessionSummary): CanonicalSession {
  ...
  return {
    ...
    date: finite(session.date) ? session.date : 0,
    day: session.day,
    durationSec: nonNegative(session.durationSec) ? session.durationSec : 0,
    ...
    sessionFatigue: session.sessionFatigue,
  };
}
...
export function normalizeReadiness(readiness: Readiness | undefined): Readiness {
  if (!readiness) return {};
  return {
    sleepHours: finite(readiness.sleepHours) && readiness.sleepHours >= 0 && readiness.sleepHours <= 24 ? readiness.sleepHours : undefined,
...
export function validateWorkoutLog(log: WorkoutLog): SessionValidation {
  ...
  if (!log.id) errors.push("missing log id");
  if (!log.sessionId) errors.push("missing session id");
  if (!log.exerciseId) errors.push("missing exercise id");
  ...
  for (const value of [...(log.result.reps || []), ...(log.result.seconds || []), ...(log.result.emom || [])]) {
    if (!nonNegative(value)) errors.push("negative or non-finite performance value");
  }
...
export function validateSession(session: SessionSummary): SessionValidation {
  ...
  if (!finite(session.date)) errors.push("invalid session date");
  if (!nonNegative(session.durationSec)) errors.push("invalid session duration");
  ...
  for (const log of session.logs || []) {
    const result = validateWorkoutLog(log);
    errors.push(...result.errors.map(error => `log ${log.id}: ${error}`));
    warnings.push(...result.warnings.map(warning => `log ${log.id}: ${warning}`));
  }
  return {valid: errors.length === 0, errors, warnings};
}

export function sumReps(logs: CanonicalWorkoutLog[]): number {
  return logs.reduce((sum, log) => sum + log.sets.reduce((s, set) => s + (set.reps ?? 0), 0), 0);
}

export function sumEmom(logs: CanonicalWorkoutLog[]): number {
  return logs.filter(log => log.kind === "EMOM").reduce((sum, log) => sum + log.sets.reduce((s, set) => s + (set.reps ?? 0), 0), 0);
}

export function bestSeconds(logs: CanonicalWorkoutLog[]): number {
  return logs.reduce((best, log) => Math.max(best, ...log.sets.map(set => set.seconds ?? 0)), 0);
}
