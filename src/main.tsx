...
function weeklyCoachChanges(day:DayKey){
 const phase=currentPhase(Date.now());
 const base=buildPeriodizedDay({phase,day,goals:["oap","flpu","front_lever_touch","pushups","dips"]}).blocks;
 const current=effectiveProgram(day).blocks;
 const currentById=new Map(current.map(b=>[b.id,b]));
 const changes:any[]=[];
 for(const b of base){
   const c=currentById.get(b.id); if(!c) continue;
   if((b.sets||0)!==(c.sets||0)) changes.push({id:b.id,name:b.name,before:`${b.sets||0} serie`,after:`${c.sets||0} serie`,reason:'volume'});
   if(b.target!==c.target) changes.push({id:`${b.id}-target`,name:b.name,before:b.target,after:c.target,reason:'target'});
   if((b.minutes||0)!==(c.minutes||0) && b.kind==='EMOM') changes.push({id:`${b.id}-minutes`,name:b.name,before:`${b.minutes||0} min`,after:`${c.minutes||0} min`,reason:'duration'});
 }
...
function Plan({day,setDay,refresh,remoteCatalog}:{day:DayKey;setDay:(x:DayKey)=>void;refresh:number;remoteCatalog:ExerciseCatalogItem[]}){
 ...
 return <div>
  <div className="eyebrow">PIANO</div><div className="flex items-end justify-between gap-4"><div><h1>Your program.</h1><p className="sub">{coachLinked?"Coach-managed plan.":"Your training plan."}</p></div>{!coachLinked&&<button className="secondary-cta" onClick={()=>{const snapshot=getProgramOverrides();clearAllProgramOverrides();syncProgramLayer().catch(err=>console.warn("Program sync deferred.",err));showNotice("PROGRAMMA RIPRISTINATO AI DEFAULT DEL COACH",()=>{Object.values(snapshot).forEach((o:any)=>setProgramOverride(o.exerciseId,o));syncProgramLayer().catch(err=>console.warn("Program sync deferred.",err));})}}>RESET ALL</button>}</div>
  <ProgramValidationCard refresh={refresh}/>
  ...
  <div className="mb-4 rounded-2xl border border-violet-500/15 bg-violet-500/5 p-4"><div className="flex items-start justify-between gap-3"><div><div className="section-kicker">COSA CAMBIA QUESTA SETTIMANA</div><p className="mt-1 text-[9px] leading-4 text-zinc-500">Solo le differenze rispetto al piano di riferimento. Le modifiche approvate dal Coach restano visibili qui.</p></div><span className="tag">{phaseLabel(currentPhase(Date.now()).type)}</span></div>{weeklyCoachChanges(day).length?<div className="mt-3 grid gap-2 sm:grid-cols-2">{weeklyCoachChanges(day).map(x=><div key={x.id} className="rounded-xl border border-line bg-panel p-3"><div className="text-[10px] font-extrabold">{x.name}</div><div className="mt-1 text-[9px] text-zinc-500">{x.before} → <b className="text-white">{x.after}</b></div></div>)}</div>:<div className="mt-3 text-[9px] text-zinc-600">Nessuna modifica rispetto al piano di riferimento.</div>}</div>
  ...
  <div className="grid gap-2">{p.blocks.map((b,i)=><div key={b.id} className="rounded-2xl border border-line bg-panel p-4">
   <div className="flex items-start justify-between gap-3"><div className="min-w-0"><span className="tag">{LABEL[b.kind]}</span><strong className="mt-1 block">{b.name}</strong><p className="mt-1 text-[10px] text-zinc-500">{b.kind==="EMOM"?`${b.minutes||10} min EMOM · ${b.target}${b.rest?` · ${b.rest}s rest`:""}`:`${b.sets?`${b.sets} sets · `:""}${b.target}${b.rest?` · ${b.rest}s rest`:""}`}</p>{overrides[b.id]&&<span className="mt-2 inline-block text-[8px] font-bold tracking-[.12em] text-violet2">CUSTOMIZED</span>}</div>{!coachLinked&&<button className="mini-btn shrink-0" aria-label={`Edit ${b.name}`} onClick={()=>{track("program_edit_opened",{exerciseId:b.id,day});setNotice("");setEditing(b.id)}}>⋯</button>}</div>
   {editing===b.id&&<ProgramEditor block={b} catalog={catalog} onClose={()=>setEditing(null)} onSaved={(undoFn)=>showNotice("PROGRAMMA SALVATO — PROSSIMA SESSIONE AGGIORNATA",undoFn)}/>}
  </div>)}</div>
 </div>;
}
...
function ProgramValidationCard({refresh}:{refresh:number}){
 ...
 const signals=report.signals.slice(0,expanded?12:4);
 const totalSets=report.days.reduce((a,d)=>a+d.adjustedSets,0);
 const totalFatigue=report.days.reduce((a,d)=>a+d.fatigueLoad,0);
 const totalGrip=report.days.reduce((a,d)=>a+d.gripScore,0);
 return <div className="mb-5 rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4">
  <div className="flex items-start justify-between gap-3"><div><div className="section-kicker">PROGRAM VALIDATION</div><div className="mt-1 text-sm font-extrabold">Weekly simulation</div><p className="mt-1 text-[10px] leading-5 text-zinc-500">Checks planned workload, recovery spacing, grip demand, overlap and sequencing before you run the week.</p></div><div className="text-right"><div className={`text-lg font-extrabold ${scoreTone}`}>{report.score}</div><div className="text-[8px] font-bold tracking-[.12em] text-zinc-600">{report.severity}</div></div></div>
  <div className="mt-4 grid gap-2 sm:grid-cols-3"><div className="rounded-xl border border-line bg-panel p-3"><div className="field-label">SERIE PESATE</div><div className="mt-1 text-lg font-extrabold">{totalSets.toFixed(1)}</div></div><div className="rounded-xl border border-line bg-panel p-3"><div className="field-label">CARICO DI FATICA</div><div className="mt-1 text-lg font-extrabold">{totalFatigue.toFixed(1)}</div></div><div className="rounded-xl border border-line bg-panel p-3"><div className="field-label">IMPEGNO DELLA PRESA</div><div className="mt-1 text-lg font-extrabold">{totalGrip.toFixed(1)}</div></div></div>
  <div className="mt-4 grid gap-2 sm:grid-cols-2">{signals.map((s,i)=><div key={`${s.kind}-${s.title}-${i}`} className="rounded-xl border border-line bg-panel2 p-3"><div className="flex items-center justify-between gap-2"><span className={`text-[8px] font-extrabold tracking-[.12em] ${s.severity==="HIGH"?"text-rose-300":s.severity==="WATCH"?"text-amber-300":"text-zinc-600"}`}>{s.severity}</span><span className="text-[8px] text-zinc-600">{s.kind}</span></div><div className="mt-1 text-[10px] font-bold text-zinc-300">{s.title}</div><p className="mt-1 text-[9px] leading-4 text-zinc-500">{s.detail}</p></div>)}</div>
  {report.signals.length>0&&<button className="secondary-cta mt-3 w-full" onClick={()=>{setExpanded(v=>!v);track("program_validation_toggled",{expanded:!expanded,signals:report.signals.length})}}>{expanded?"MOSTRA MENO SEGNALI":`MOSTRA TUTTI I SEGNALI · ${report.signals.length}`}</button>}
  {report.strengths.length>0&&<div className="mt-3 rounded-xl border border-emerald-500/10 bg-emerald-500/5 p-3"><div className="section-kicker text-emerald-300">STRENGTHS</div><div className="mt-2 grid gap-1">{report.strengths.map(x=><div key={x} className="text-[9px] text-zinc-400">{x}</div>)}</div></div>}
  <div className="mt-3 text-[8px] leading-4 text-zinc-600">Heuristic only · a signal triggers review, not an automatic program change.</div>
 </div>;
}
...
function CoachLinkCard({onLinked}:{onLinked:()=>void}){
 ...
 return <div className="mt-5 rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4"><div className="section-kicker">COLLEGAMENTO COACH</div>{coach?<div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-sm font-extrabold">{coach.display_name||'Your coach'}</div><div className="mt-1 text-[9px] text-zinc-600">LINKED {new Date(coach.linked_at).toLocaleDateString()}</div></div><button className="secondary-cta" disabled={busy} onClick={unlink}>{busy?'WORKING…':'DISCONNECT COACH'}</button></div>:<><p className="mt-2 text-[10px] leading-5 text-zinc-500">Have your coach send you their 10-character code. Linking does not change your workout history.</p><div className="mt-3 flex gap-2"><input className="flex-1 rounded-xl border border-line bg-panel2 p-3 font-mono text-sm uppercase tracking-[.18em]" maxLength={10} value={code} onChange={e=>setCode(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g,''))} placeholder="CODICE COACH"/><button className="primary-cta" disabled={busy||code.length!==10} onClick={link}>{busy?'LINKING…':'LINK'}</button></div></>}{msg&&<div className={`mt-2 text-[9px] ${msg.includes('FAILED')?'text-rose-300':'text-emerald-300'}`}>{msg}</div>}</div>;
}
...
function ProgramEditor({block,catalog,onClose,onSaved}:{block:ExerciseBlock;catalog:{id:string;name:string;detail?:string;kind:BlockKind;bandOptions?:Band[];category?:string;skill?:string;pattern?:string;difficulty?:number;sideMode?:string;defaultTarget?:string;restSec?:number}[];onClose:()=>void;onSaved:(undo?:()=>void)=>void}){
 ...
 return <div className="mt-4 rounded-xl border border-violet-500/15 bg-panel2 p-4">
  ...
  <label className="mt-3 block"><span className="field-label">ESERCIZIO / VARIANTE</span><select value={catalogId} onChange={e=>choose(e.target.value)}>{compatible.map(x=><option key={x.id} value={x.id}>{x.name}{x.difficulty?` · D${x.difficulty}`:""}</option>)}</select><span className="mt-1 block text-[8px] text-zinc-600">Showing compatible movements by skill or movement pattern.</span></label>
  ...
  <label className="mt-3 block"><span className="field-label">NOTA DEL COACH</span><input value={detail} onChange={e=>setDetail(e.target.value)} placeholder="Optional cue"/></label>
...
function CoachTimeline({sessions,program}:{sessions:any[];program:any}){const rows=coachTimelineRows(sessions,program);return <div className="mt-5 rounded-2xl border border-violet-500/15 bg-violet-500/5 p-4"><div className="flex items-end justify-between gap-3"><div><div className="section-kicker">STORICO ATLETA</div><div className="mt-1 text-[9px] text-zinc-500">Last exposures, best result and short-term direction.</div></div><span className="tag">ULTIME 6</span></div>{!rows.length?<div className="mt-3 text-xs text-zinc-600">Not enough exercise data yet.</div>:<div className="mt-3 grid gap-2 lg:grid-cols-2">{rows.map((r:any)=><div key={r.id} className="rounded-xl border border-line bg-panel p-3"><div className="flex items-start justify-between gap-3"><div><div className="text-[8px] text-zinc-600">{r.exposures} EXPOSURES</div><div className="mt-1 text-sm font-extrabold">{r.name}</div></div><span className={`text-[8px] font-extrabold tracking-[.12em] ${r.signal==="PROGRESSING"?"text-emerald-400":r.signal==="REGRESSING"?"text-amber-300":"text-violet2"}`}>{r.signal}</span></div><div className="mt-3 grid grid-cols-3 gap-2"><div><div className="field-label">ULTIMO</div><div className="mt-1 text-sm font-bold">{r.latest.toFixed(r.unit==="s"?1:0)} <span className="text-[8px] text-zinc-600">{r.unit}</span></div></div><div><div className="field-label">MIGLIORE</div><div className="mt-1 text-sm font-bold">{r.best.toFixed(r.unit==="s"?1:0)} <span className="text-[8px] text-zinc-600">{r.unit}</span></div></div><div><div className="field-label">VARIAZIONE</div><div className="mt-1 text-sm font-bold">{r.exposures>1?`${r.delta>=0?"+":""}${r.delta.toFixed(0)}%`:"—"}</div></div></div></div>)}</div>}</div>}
...
function SkillGraphPanel({catalog,blocks,insights,onPromote}:{catalog:ExerciseCatalogItem[];blocks:ExerciseBlock[];insights:SkillInsight[];onPromote:(view:SkillGraphView)=>void}){
 ...
 return <div className="mt-5 rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4">
  <div className="flex items-end justify-between gap-3"><div><div className="section-kicker">SKILL GRAPH</div><div className="mt-1 text-[9px] text-zinc-500">Mechanical roadmap + evidence gate. PRONTO means the current rung has passed Skill Intelligence; promotion still requires Coach confirmation.</div></div><span className="tag">PATHWAYS</span></div>
...
function CoachDecisionCenter({athleteId,insights,decisions,onRecorded}:{athleteId:string;insights:any[];decisions:any[];onRecorded:()=>void}){
 ...
 return <div className="mt-5 rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4">
   <div className="flex items-end justify-between gap-3">
     <div><div className="section-kicker">CENTRO DECISIONI COACH</div><div className="mt-1 text-[9px] text-zinc-500">Review the recommendation, record the human decision, and preserve the reason.</div></div>
    ...
    {rows.slice(0,8).map((r:any)=>{
      ...
      return <div key={r.id} className="rounded-xl border border-line bg-panel p-3">
       ...
       open===r.id?<div className="mt-3">
        ...
        <div className="mt-2 grid grid-cols-2 gap-2"><button className="secondary-cta" onClick={()=>resolve(r,"rejected")}>RIFIUTA</button><button className="primary-cta" onClick={()=>resolve(r,"accepted")}>ACCEPT</button></div>
...
function CoachPerformanceDashboard({sessions,program,decisions}:{sessions:any[];program:any;decisions:any[]}){
 ...
 return <div className="mt-5 rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4">
   <div className="flex items-end justify-between gap-3"><div><div className="section-kicker">ATHLETE PERFORMANCE DASHBOARD</div><div className="mt-1 text-[9px] text-zinc-500">Premium snapshot for coaching decisions — performance, consistency, recovery and workload.</div></div><span className="tag">ULTIME 6</span></div>
   <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
    <div className="rounded-xl border border-line bg-panel p-3"><div className="field-label">ESPOSIZIONI</div><div className="mt-1 text-xl font-extrabold">{logs.length}</div><div className="mt-1 text-[8px] text-zinc-600">logged exercise exposures</div></div>
...
function CoachWorkspace({remoteCatalog}:{remoteCatalog:ExerciseCatalogItem[]}){
 ...
 return <div>
   {promotion&&<div className="fixed inset-0 z-50 flex items-end bg-black/70 backdrop-blur-sm"><div className="mx-auto w-full max-w-xl rounded-t-3xl border border-line bg-panel p-5 pb-[calc(env(safe-area-inset-bottom)+20px)]"><div className="eyebrow">COACH PROMOTION REVIEW</div><h2 className="mt-2 text-2xl font-extrabold">{promotion.current?.name} → {promotion.next?.name}</h2><p className="mt-2 text-xs leading-5 text-zinc-500">Skill Intelligence has qualified the current rung. This action changes only future sessions and leaves historical logs untouched.</p><div className="mt-4 grid grid-cols-3 gap-2"><div className="rounded-xl border border-line bg-panel2 p-3"><div className="field-label">ATTUALE</div><div className="mt-1 text-[10px] font-bold">{promotion.current?.name}</div></div><div className="rounded-xl border border-line bg-panel2 p-3"><div className="field-label">STATUS</div><div className="mt-1 text-[10px] font-bold text-emerald-300">QUALIFIED</div></div><div className="rounded-xl border border-line bg-panel2 p-3"><div className="field-label">NEXT</div><div className="mt-1 text-[10px] font-bold">{promotion.next?.name}</div></div></div><div className="mt-4 grid grid-cols-2 gap-2"><button className="secondary-cta" disabled={promotionBusy} onClick={()=>setPromotion(null)}>ANNULLA</button><button className="primary-cta" disabled={promotionBusy} onClick={()=>promoteSkill(promotion)}>{promotionBusy?"PUBLISHING…":"PROMOTE TO NEXT RUNG"}</button></div></div></div>}
   <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><div className="eyebrow">SPAZIO COACH</div><h1>Manage athletes.</h1><p className="sub">One place to review evidence, prescribe training and publish decisions.</p></div><div className="rounded-2xl border border-violet-500/20 bg-violet-500/5 px-4 py-3 md:min-w-[220px]"><div className="flex items-center justify-between gap-4"><div className="field-label !mb-0">YOUR COACH CODE</div><span className="text-[8px] font-bold tracking-[.12em] text-emerald-400">PAIRING ONLY</span></div><div className="mt-1 flex items-center justify-between gap-3"><div className="font-mono text-lg font-extrabold tracking-[.18em]">{coachCode||"—"}</div><button className="secondary-cta !px-3 !py-2" disabled={!coachCode} onClick={copyCoachCode}>COPIA</button></div><div className="mt-2 text-[8px] leading-4 text-zinc-600">Share this code with an athlete. It does not grant access by itself.</div></div></div><div className="mt-4 sticky top-[70px] z-20 rounded-2xl border border-line bg-ink/95 p-2 backdrop-blur-xl"><div className="grid grid-cols-3 gap-1"><button className="secondary-cta !py-2" onClick={()=>document.getElementById("coach-intelligence")?.scrollIntoView({behavior:"smooth",block:"start"})}>OVERVIEW</button><button className="secondary-cta !py-2" onClick={()=>document.getElementById("coach-program")?.scrollIntoView({behavior:"smooth",block:"start"})}>PROGRAM</button><button className="secondary-cta !py-2" onClick={()=>document.getElementById("coach-history")?.scrollIntoView({behavior:"smooth",block:"start"})}>HISTORY</button></div></div>
   ...
   {!athletes.length?<div className="mt-6 grid gap-3 md:grid-cols-[1.2fr_.8fr]"><div className="rounded-2xl border border-line bg-panel p-5"><div className="section-kicker">YOUR COACHING ROSTER</div><h2 className="mt-2 text-2xl font-extrabold tracking-tight">No athletes yet.</h2><p className="mt-2 max-w-xl text-xs leading-5 text-zinc-500">Give your 10-character Coach Code to an athlete. Once they connect from Reports → Coach, their training evidence will appear here automatically.</p><div className="mt-5 rounded-xl border border-line bg-panel2 p-4"><div className="field-label">NEXT STEP</div><div className="mt-2 text-sm font-bold">Wait for the first athlete connection.</div><div className="mt-1 text-[9px] leading-4 text-zinc-600">The Coach does not need to create athletes manually.</div></div></div><div className="rounded-2xl border border-violet-500/20 bg-violet-500/5 p-5"><div className="section-kicker">COACHING LOOP</div><div className="mt-3 grid gap-2 text-[10px] font-bold"><div className="rounded-xl bg-panel2 p-3">01 · ATHLETE TRAINS</div><div className="rounded-xl bg-panel2 p-3">02 · EVIDENCE SYNCs</div><div className="rounded-xl bg-panel2 p-3">03 · COACH REVIEWS</div><div className="rounded-xl bg-panel2 p-3">04 · COACH PUBLISHES</div></div></div></div>:<div className="mt-6 grid gap-3 md:grid-cols-[240px_1fr]">
    ...
    <div className="min-w-0">
      ...
      <div id="coach-intelligence" className="mt-4 rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4"><div className="flex items-end justify-between gap-3"><div><div className="section-kicker">INTELLIGENZA COACH</div><div className="mt-1 text-[9px] text-zinc-500">Decision support from readiness, pain, recent volume and adherence. The Coach never changes the plan automatically.</div></div><span className="tag">HUMAN REVIEW</span></div><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{decisions.map((d,i)=><div key={i} className="rounded-xl border border-line bg-panel p-3"><div className="flex items-start justify-between gap-2"><div className="text-[10px] font-extrabold">{d.title}</div><span className={`text-[8px] font-extrabold tracking-[.12em] ${d.level==="RED"?"text-rose-400":d.level==="YELLOW"?"text-amber-300":"text-emerald-400"}`}>{d.level}</span></div><div className="mt-2 text-[9px] leading-4 text-zinc-500">{d.detail}</div><div className="mt-2 text-[8px] font-bold tracking-[.12em] text-violet2">{d.action}</div></div>)}</div></div>
...
function CoachProgramEditor({athleteId,day,block,remoteBlock,sortOrder,catalog,onClose,onSaved}:{athleteId:string;day:DayKey;block:ExerciseBlock;remoteBlock:any;sortOrder:number;catalog:{id:string;name:string;detail?:string;kind:BlockKind;bandOptions?:Band[];category?:string;skill?:string;pattern?:string;difficulty?:number;sideMode?:string;defaultTarget?:string;restSec?:number}[];onClose:()=>void;onSaved:(message:string)=>void}){
 ...
 return <div className="mt-4 rounded-xl border border-violet-500/15 bg-panel2 p-4"><div className="flex items-center justify-between"><div className="field-label">COACH EDITOR</div><button className="text-[9px] font-bold text-zinc-600" onClick={onClose}>CHIUDI</button></div>{error&&<div className="mt-3 rounded-lg border border-rose-500/20 bg-rose-500/5 p-2 text-[9px] text-rose-300">{error}</div>}<label className="mt-3 block"><span className="field-label">ESERCIZIO / VARIANTE</span><select value={catalogId} onChange={e=>choose(e.target.value)} disabled={busy}>{compatible.map(x=><option key={x.id} value={x.id}>{x.name}{x.difficulty?` · D${x.difficulty}`:""}</option>)}</select><span className="mt-1 block text-[8px] text-zinc-600">Compatible by skill or movement pattern. The catalog is the source of prepared alternatives.</span></label><div className="mt-3 grid grid-cols-3 gap-2"><Field label="SERIE" value={sets} set={setSets} placeholder="3"/><Field label="TARGET" value={target} set={setTargetLocal} placeholder="8–10"/><Field label="RECUPERO (S)" value={rest} set={setRestLocal} placeholder="90"/></div>{(currentCatalog?.bandOptions||block.bandOptions||[]).length>0&&<div className="mt-3"><BandSelect label="DEFAULT LOOP" value={band} set={setBand} options={(currentCatalog?.bandOptions||block.bandOptions||[]) as string[]}/></div>}<label className="mt-3 block"><span className="field-label">NOTA DEL COACH</span><input value={detail} onChange={e=>setDetail(e.target.value)} placeholder="Optional cue" disabled={busy}/></label><div className="mt-4 grid grid-cols-2 gap-2"><button className="secondary-cta" disabled={busy} onClick={reset}>RIPRISTINA PREDEFINITO</button><button className="primary-cta" disabled={busy} onClick={save}>{busy?"SAVING…":"PUBBLICA MODIFICA"}</button></div><div className="mt-2 text-[8px] leading-4 text-zinc-600">Only the future prescription changes. Workout history is immutable.</div></div>;
}
...
function RecoveryTrend(){const ss=getSessions().slice(-7);const rows=ss.map(s=>({date:new Date(s.date).toLocaleDateString('it-IT'),sleep:s.readiness.sleepHours,energy:s.readiness.energy,pain:Math.max(s.readiness.wristPain||0,s.readiness.elbowPain||0)}));return <div className="mt-8 rounded-2xl border border-line bg-panel p-4"><div className="section-kicker">RECOVERY — LAST 7</div><div className="mt-3 grid gap-2">{rows.slice().reverse().map(r=><div key={r.date} className="history-row"><span>{r.date}</span><strong>{r.sleep?`${r.sleep.toFixed(1)}h sleep`:'—'}</strong><span>Energy {r.energy??'—'}/5 · Pain {r.pain}/5</span></div>)}{!rows.length&&<p className="text-xs text-zinc-600">Nessuno storico del recupero.</p>}</div></div>}
...
function readinessStatus(r:Readiness){
 ...
 if(Number.isFinite(energy)||Number.isFinite(sleep))return {label:"PRONTO",tone:"text-emerald-300",detail:"Recovery data supports the planned session."};
...
function Today({day,setDay,start,draft,onResume}:{day:DayKey;setDay:(x:DayKey)=>void;start:()=>void;draft:any;onResume:()=>void}){
 ...
 return <div>
  <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
   <div><div className="eyebrow">OGGI</div><h1>{p.title}</h1><p className="sub">{p.subtitle}</p>{push&&<div className="mt-2 text-[10px] text-violet2">PUSH GOALS · 100 PUSH-UPS / 50 DIPS</div>}</div>
   <button className="primary-cta w-full md:w-auto" onClick={draft?onResume:start}>{draft?"RESUME WORKOUT":"START WORKOUT"}</button>
  </div>
  <div className="my-7 grid grid-cols-7 gap-1">{DAYS.map(d=><button key={d} onClick={()=>setDay(d)} className={`rounded-lg border px-1 py-2 text-[9px] font-extrabold ${day===d?"border-transparent bg-violet-600 text-white":"border-line bg-panel text-zinc-600"}`}>{d.slice(0,3).toUpperCase()}</button>)}</div>
  <div className="mb-4 rounded-2xl border border-violet-500/15 bg-violet-500/5 p-4"><div className="flex items-start justify-between gap-3"><div><div className="section-kicker">COSA CAMBIA QUESTA SETTIMANA</div><p className="mt-1 text-[9px] leading-4 text-zinc-500">Solo le differenze rispetto al piano di riferimento. Le modifiche approvate dal Coach restano visibili qui.</p></div><span className="tag">{phaseLabel(currentPhase(Date.now()).type)}</span></div>{weeklyCoachChanges(day).length?<div className="mt-3 grid gap-2 sm:grid-cols-2">{weeklyCoachChanges(day).map(x=><div key={x.id} className="rounded-xl border border-line bg-panel p-3"><div className="text-[10px] font-extrabold">{x.name}</div><div className="mt-1 text-[9px] text-zinc-500">{x.before} → <b className="text-white">{x.after}</b></div></div>)}</div>:<div className="mt-3 text-[9px] text-zinc-600">Nessuna modifica rispetto al piano di riferimento.</div>}</div>
  <div className="mb-3 text-[9px] font-extrabold tracking-[.16em] text-zinc-600">TEMPO</div>
  ...
  <div className="mt-4 grid gap-2 md:grid-cols-2">
    {last?<div className="rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4"><div className="section-kicker">LAST TEMPO</div><div className="mt-2 flex flex-wrap gap-3 text-[10px] text-zinc-400"><span>{Math.round(last.durationSec/60)} min</span><span>{last.totalReps} reps</span><span>{last.emomReps} EMOM</span></div></div>:<div className="rounded-2xl border border-line bg-panel p-4"><div className="section-kicker">FIRST TEMPO</div><p className="mt-2 text-[10px] leading-5 text-zinc-500">Your performance history will appear here after the first completed session.</p></div>}
...
function Progress({refresh}:{refresh:number}){
 ...
 return <div>
  ...
  <div className="mt-8"><div className="flex items-end justify-between"><div><div className="section-kicker">ARCHIVIO PR</div><p className="mt-1 text-[10px] text-zinc-600">Best recorded performance, not today's target.</p></div></div>
...
function EffortGuide(){return <div className="mt-3 rounded-xl border border-line bg-panel2 p-3 text-[9px] leading-4 text-zinc-500"><div className="field-label">SFORZO · USA SEMPRE LA STESSA SCALA</div><div className="mt-2 grid gap-2 sm:grid-cols-2"><div><b className="text-zinc-300">RIR</b><div className="mt-1">Quante reps pulite avresti ancora fatto? 0 nessuna · 1 una · 2 due · 3 tre o più.</div></div><div><b className="text-zinc-300">FATICA</b><div className="mt-1">Quanto è stata pesante la serie? 1 facile · 3 impegnativa · 5 quasi esausto.</div></div></div><div className="mt-2 text-[8px] text-zinc-600">Non serve essere precisi al decimale: il Coach usa questi dati insieme a reps, qualità e recupero.</div></div>}
function EffortPicker({rir,setRir,fatigue,setFatigue}:{rir:string;setRir:(v:string)=>void;fatigue:string;setFatigue:(v:string)=>void}){return <div className="mt-4 rounded-2xl border border-line bg-panel p-3"><div className="grid gap-4 sm:grid-cols-2"><div><div className="flex items-center justify-between"><span className="field-label">QUANTE REPS AVEVI ANCORA?</span>{rir!==''&&<span className="text-[8px] font-bold text-violet2">{rirLabel(Number(rir))}</span>}</div><div className="mt-2 grid grid-cols-4 gap-2">{RIR_OPTIONS.map(v=><button type="button" key={v} onClick={()=>setRir(rir===String(v)?'':String(v))} aria-pressed={rir===String(v)} className={`rounded-xl border px-2 py-3 text-[10px] font-extrabold transition ${rir===String(v)?'border-violet-400 bg-violet-500/10 text-violet2':'border-line bg-panel2 text-zinc-500 hover:text-zinc-300'}`}>{v===3?'3+':v}</button>)}</div></div><div><div className="flex items-center justify-between"><span className="field-label">QUANTO È STATA DURA?</span>{fatigue!==''&&<span className="text-[8px] font-bold text-violet2">{fatigueLabel(Number(fatigue))}</span>}</div><div className="mt-2 grid grid-cols-5 gap-1.5">{FATIGUE_OPTIONS.map(v=><button type="button" key={v} onClick={()=>setFatigue(fatigue===String(v)?'':String(v))} aria-pressed={fatigue===String(v)} className={`rounded-xl border px-1 py-3 text-[10px] font-extrabold transition ${fatigue===String(v)?'border-violet-400 bg-violet-500/10 text-violet2':'border-line bg-panel2 text-zinc-500 hover:text-zinc-300'}`}>{v}</button>)}</div><div className="mt-1 text-right text-[8px] text-zinc-600">1 facile · 5 quasi esausto</div></div></div></div>}

function ExerciseHistoryModal({id,onClose}:{id:string;onClose:()=>void}){const rows=getLogs().filter(l=>l.exerciseId===id&&l.status!=="skipped").sort((a,b)=>b.date-a.date).slice(0,12);return <div className="fixed inset-0 z-50 overflow-y-auto bg-ink/95 backdrop-blur"><div className="mx-auto max-w-2xl px-4 py-8 pb-28"><div className="flex items-start justify-between gap-3"><div><div className="eyebrow">STORICO ESERCIZIO</div><h2 className="mt-2 text-3xl font-extrabold">{exerciseNameFor(id)}</h2><p className="sub">Most recent performance, target context, and how the movement is moving.</p></div><button className="secondary-cta" onClick={onClose}>CHIUDI</button></div><div className="mt-5 grid gap-2">{rows.map(l=><div key={l.id} className="rounded-2xl border border-line bg-panel p-4"><div className="flex items-center justify-between"><span className="field-label">{new Date(l.date).toLocaleDateString()}</span><span className="text-[9px] text-zinc-600">{l.status.toUpperCase()}</span></div><div className="mt-2 text-sm font-bold">{l.kind==="EMOM"?`EMOM ${(l.result.emom||[]).join(" / ")}`:l.result.seconds?.length?`HOLD ${(l.result.seconds||[]).map(x=>x.toFixed(1)).join(" / ")}s`:`SETS ${(l.result.reps||[]).join(" / ")}`}</div><div className="mt-2 text-[9px] text-zinc-600">{l.result.band&&l.result.band!=="None"?`LOOP ${l.result.band} · `:""}{l.result.rir!==undefined?`RIR ${l.result.rir} · `:""}{l.result.fatigue!==undefined?`FATIGUE ${l.result.fatigue}/5`:""}{l.result.note?` · ${l.result.note}`:""}</div></div>)}{!rows.length&&<div className="rounded-2xl border border-line bg-panel p-4 text-xs text-muted">No history for this movement.</div>}</div></div></div>}
...
function GoalProgressCard({goal,target,label}:{goal:"push-up"|"dips";target:number;label:string}){
 ...
 return <div className="rounded-2xl border border-line bg-panel p-4">
   ...
   <div className="mt-3 grid grid-cols-3 gap-2 text-[9px]"><div><span className="field-label">MIGLIORE</span><b className="mt-1 block text-white">{best||"—"}</b></div><div><span className="field-label">LAST</span><b className="mt-1 block text-white">{last.length?last[last.length-1]:"—"}</b></div><div><span className="field-label">TREND</span><b className="mt-1 block text-white">{delta===null?"—":`${delta>=0?"+":""}${delta.toFixed(0)}%`}</b></div></div>
...
function CoachGoalsDashboard(){
 ...
 return <section className="mt-6 rounded-2xl border border-violet-500/15 bg-violet-500/5 p-4">
   <div className="flex items-end justify-between gap-3">
    <div><div className="section-kicker">OBIETTIVI COACH</div><p className="mt-1 text-[9px] text-zinc-500">Performance targets tracked from benchmark-quality exposures.</p></div>
...
function GoalAnalyticsCard({snapshot:g}:{snapshot:GoalPerformanceSnapshot}){
 ...
 const status=g.status==='REALIZING'?'TARGET REACHED':g.status==='PROGRESSING'?'PROGRESSING':g.status==='REGRESSING'?'CHECK RECOVERY':g.status==='STALLED'?'STABLE':'BUILDING';
 const tone=status==='TARGET REACHED'?'text-emerald-400':status==='PROGRESSING'?'text-violet2':status==='CHECK RECOVERY'?'text-amber-300':'text-zinc-500';
 ...
 return <div className="rounded-xl border border-line bg-panel p-3">
   ...
   <div className="mt-3 h-2 overflow-hidden rounded-full bg-zinc-900"><div className="h-full rounded-full bg-violet-500 transition-all" style={{width:`${pct}%`}}/></div>
   <div className="mt-3 grid grid-cols-3 gap-2 text-[9px]"><div><span className="field-label">ULTIMO</span><b className="mt-1 block text-white">{g.current||'—'}</b></div><div><span className="field-label">TREND</span><b className="mt-1 block text-white">{g.trendPct===0?'—':`${g.trendPct>=0?'+':''}${g.trendPct.toFixed(0)}%`}</b></div><div><span className="field-label">EVIDENCE</span><b className="mt-1 block text-white">{g.exposures}</b></div></div>
...
function WeeklyReview({sessions}:{sessions:SessionSummary[]}){
 ...
 return <section className="mt-6 rounded-2xl border border-line bg-panel p-4"><div className="flex items-end justify-between gap-3"><div><div className="section-kicker">WEEKLY REVIEW</div><p className="mt-1 text-[9px] text-zinc-600">The smallest set of signals that tells you whether the week is trending well.</p></div><span className="tag">{sessions.length}/6 SESSIONS</span></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><Metric label="TEMPO" value={formatClock(duration) as any}/><Metric label="RIPETIZIONI" value={reps}/><Metric label="EMOM" value={emom}/><Metric label="PRONTO DAYS" value={ready}/></div><div className="mt-4 grid gap-2 sm:grid-cols-2"><div className="rounded-xl border border-line bg-panel2 p-3"><span className="field-label">FATIGUE</span><div className="mt-1 text-sm font-extrabold">{avgFat!==null?`${avgFat.toFixed(1)} / 5`:`—`}</div><p className="mt-1 text-[8px] text-zinc-600">Average logged fatigue across the week.</p></div><div className="rounded-xl border border-line bg-panel2 p-3"><span className="field-label">COACH TAKEAWAY</span><div className="mt-1 text-sm font-extrabold">{sessions.length>=4&&avgFat!==null&&avgFat<3.5?"Stable training week":"Build evidence before adding more volume."}</div><p className="mt-1 text-[8px] text-zinc-600">Use progression proposals, not emotion, to edit the plan.</p></div></div></section>
}
...
function AnalyticsPanel(){
 ...
 return <div className="mt-8 rounded-2xl border border-line bg-panel p-4">
  <div className="flex items-start justify-between gap-3"><div><div className="section-kicker">USAGE ANALYTICS</div><p className="mt-1 text-[9px] leading-4 text-zinc-600">Local anonymous product-usage intelligence. Use this to spot adoption, funnel drop-off and exercise friction before changing the product.</p></div><span className="tag">{summary.totalEvents} EVENTS</span></div>
  ...
  <div className="mt-4 grid gap-2 sm:grid-cols-4">
   <div className="rounded-xl border border-line bg-panel2 p-3"><span className="field-label">WORKOUT COMPLETION</span><div className="mt-1 text-lg font-extrabold">{intel.completion.rate.toFixed(0)}%</div><div className="mt-1 text-[8px] text-zinc-600">{intel.completion.completed}/{intel.completion.started} started</div></div>
   <div className="rounded-xl border border-line bg-panel2 p-3"><span className="field-label">READINESS</span><div className="mt-1 text-lg font-extrabold">{intel.readiness.completionRate.toFixed(0)}%</div><div className="mt-1 text-[8px] text-zinc-600">submitted vs skipped</div></div>
   <div className="rounded-xl border border-line bg-panel2 p-3"><span className="field-label">COACH ACCEPT</span><div className="mt-1 text-lg font-extrabold">{intel.coach.decisionRate.toFixed(0)}%</div><div className="mt-1 text-[8px] text-zinc-600">{intel.coach.accepted}/{intel.coach.accepted+intel.coach.rejected} decisions</div></div>
   <div className="rounded-xl border border-line bg-panel2 p-3"><span className="field-label">DRAFT RATE</span><div className="mt-1 text-lg font-extrabold">{intel.recovery.draftRate.toFixed(0)}%</div><div className="mt-1 text-[8px] text-zinc-600">unfinished workouts</div></div>
  </div>
  <div className="mt-4 grid gap-3 lg:grid-cols-2">
   <div className="rounded-xl border border-line bg-panel2 p-3"><div className="section-kicker">WORKOUT FUNNEL</div><div className="mt-3 grid gap-2">{intel.funnel.map((x,i)=><div key={x.name}><div className="flex items-center justify-between text-[9px]"><span className="text-zinc-400">{x.name}</span><b>{x.count}{i>0?` · ${x.rate.toFixed(0)}% vs previous`:''}</b></div><div className="mt-1 h-1 overflow-hidden rounded-full bg-black/30"><div className="h-full rounded-full bg-violet-500" style={{width:`${Math.min(100,x.rate||0)}%`}}/></div></div>)}</div></div>
  ...
  <div className="mt-3 grid gap-3 lg:grid-cols-2">
   <div className="rounded-xl border border-line bg-panel2 p-3"><div className="section-kicker">FRIZIONE ESERCIZI</div><p className="mt-1 text-[8px] text-zinc-600">Started → skipped/substituted. High friction is a signal to review UX or exercise selection.</p>{riskExercises.length?<div className="mt-3 grid gap-2">{riskExercises.map(x=><div key={x.exerciseId} className="rounded-lg border border-line p-2"><div className="flex items-center justify-between text-[9px]"><b>{x.exerciseId}</b><span className="text-amber-300">{x.frictionRate.toFixed(0)}% friction</span></div><div className="mt-1 text-[8px] text-zinc-600">started {x.started} · complete {x.completed} · skipped {x.skipped} · substituted {x.substituted}</div></div>)}</div>:<div className="mt-3 text-[9px] text-zinc-600">No high-friction exercises in this window.</div>}</div>
   <div className="rounded-xl border border-line bg-panel2 p-3"><div className="section-kicker">SEGNALI DI RECUPERO / ABBANDONO</div><div className="mt-3 grid gap-1 text-[9px]"><div className="flex justify-between"><span className="text-zinc-400">Resumed workouts</span><b>{intel.recovery.resumed}</b></div><div className="flex justify-between"><span className="text-zinc-400">Saved drafts</span><b>{intel.recovery.draftSaved}</b></div><div className="flex justify-between"><span className="text-zinc-400">Discarded workouts</span><b>{intel.recovery.discarded}</b></div><div className="flex justify-between"><span className="text-zinc-400">Timer skips</span><b>{intel.timer.skips}</b></div><div className="flex justify-between"><span className="text-zinc-400">Program edits</span><b>{intel.program.edits}</b></div></div></div>
  </div>
  ...
  <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3"><button className="secondary-cta" onClick={()=>{track("analytics_exported");doExport()}}>ESPORTA</button><button className="secondary-cta" onClick={()=>{clear();track("analytics_cleared")}}>SVUOTA</button><div className="flex items-center justify-center text-[8px] text-zinc-600">Window: {days} days</div></div>
 </div>
}
...
function AthleteProfileEditor(){
 ...
 return <div className="mt-8 rounded-2xl border border-line bg-panel p-4">
  ...
  <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
   ...
   <div className="rounded-xl border border-line bg-panel2 p-3"><span className="field-label">PESO ATTUALE</span><div className="mt-1 text-lg font-extrabold">{typeof currentWeight==="number"?`${currentWeight.toFixed(1)} kg`:"—"}</div><div className="mt-1 text-[8px] text-zinc-600">Latest readiness check-in</div></div>
...
function WorkoutPlayer({state,setState,refresh}:{state:any;setState:(x:any)=>void;refresh:()=>void}){
 ...
 return <div className="fixed inset-0 z-50 flex min-h-[100dvh] flex-col bg-ink text-white">
  ...
  <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain"><div className="mx-auto flex min-h-full max-w-4xl flex-col px-4 py-5 pb-28 sm:py-7 sm:pb-32">
   {active.index<0?<WarmupPlayer steps={ep.warmup} sound={sound} vibration={vibration} onDone={()=>setActive((a:any)=>({...a,index:0}))}/>:transitionNext!==null?<TransitionRecovery seconds={180} sound={sound} vibration={vibration} nextLabel={ep.blocks[transitionNext]?.name||"next exercise"} onDone={finishTransition}/>:<><div className="mb-4 rounded-2xl border border-violet-500/15 bg-violet-500/5 p-3"><div className="flex items-center justify-between gap-3"><div><div className="section-kicker">{flowCopy.phase} · {flowCopy.day}</div><div className="mt-1 text-[12px] font-extrabold">{flowCopy.headline}</div></div><span className="tag">OGGI</span></div><p className="mt-2 text-[9px] leading-4 text-zinc-500">{block?.detail}</p></div><div className="workout-context"><div><span className="tag">{LABEL[block!.kind]}</span><div className="mt-1 text-[9px] font-bold tracking-[.12em] text-zinc-600">EXERCISE {active.index+1} OF {total}</div></div><div className="workout-context-progress"><div className="workout-context-progress-bar"><span style={{width:`${Math.round(((active.index+1)/total)*100)}%`}}/></div><span>{Math.round(((active.index+1)/total)*100)}%</span></div></div><h2 className="mt-3 text-4xl font-extrabold tracking-tight sm:text-6xl">{currentVariantFor(block!.id,block!.name)}</h2><div className="mt-2 flex items-center gap-2 text-[9px] text-zinc-600"><Timer size={12}/><span>{block!.kind==="EMOM"?`${block!.minutes||10} min EMOM`:`${block!.rest||0}s recovery between sets`}</span></div><BlockPlayer key={block!.id} block={block!} day={active.day} vibration={vibration} sound={sound} existing={active.logs.find((x:WorkoutLog)=>x.exerciseId===block!.id)} onComplete={next} onTick={tick} onStarted={()=>{setExerciseStarted(true);if(block&&startedExerciseRef.current!==block.id){startedExerciseRef.current=block.id;track("exercise_started",{day:active.day,exerciseId:block.id,kind:block.kind})}}} onProgress={(partial)=>setActive((a:any)=>({...a,logs:[...a.logs.filter((x:WorkoutLog)=>x.exerciseId!==block!.id),partial]}))}/></>}
  </div></div>
  ...
  {skipAsk&&<div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm"><div className="w-full max-w-sm rounded-2xl border border-line bg-panel p-5"><div className="eyebrow">SKIP</div><h2 className="mt-2 text-xl font-extrabold">Skip this exercise?</h2><p className="mt-2 text-xs text-muted">It will be recorded as skipped and the workout will continue after the 3-minute transition recovery.</p><div className="mt-5 grid grid-cols-2 gap-2"><button className="secondary-cta" onClick={()=>setSkipAsk(false)}>ANNULLA</button><button className="primary-cta" onClick={confirmSkip}>SKIP</button></div></div></div>}
...
function progressionCount(blockId:string, block:ExerciseBlock){
 ...
 if(streak>=required)return {state:"PRONTO",label:"PRONTO FOR COACH REVIEW",detail:`Criterion reached in ${required} consecutive qualifying exposures.`};
...
function TargetPanel({block,target,onChange}:{block:ExerciseBlock;target:number;onChange:(value:number)=>void}){
 ...
 return <div className="rounded-2xl border border-line bg-panel p-4"><div className="grid grid-cols-2 gap-4"><div><span className="field-label">TARGET RANGE</span><div className="mt-1 text-sm font-bold">{block.target}</div></div><div className="text-right"><span className="field-label">TODAY'S GOAL</span><div className="mt-1 flex items-center justify-end gap-2"><button className="mini-btn" disabled={target<=r.min} onClick={()=>onChange(target-1)}><Minus size={14}/></button><b className="w-8 text-center">{target}</b><button className="mini-btn" disabled={target>=r.max} onClick={()=>onChange(target+1)}><Plus size={14}/></button></div></div></div></div>
}
function SetBlock({block,day,onComplete,onStarted,existing,onProgress,sound,vibration}:{block:ExerciseBlock;day:DayKey;onComplete:(l:WorkoutLog)=>void;onStarted:()=>void;existing?:WorkoutLog;onProgress:(l:WorkoutLog)=>void;sound:boolean;vibration:boolean}){
 ...
 const finish=(status:BlockStatus)=>{setTodayTarget(block.id,target,r.min,r.max);onComplete(makeLog(status,reps));};
 const diff=last!==undefined&&input!==""?Number(input)-last:null;
 return <div className="flex flex-1 flex-col pt-5"><TargetPanel block={block} target={target} onChange={setTarget}/><div className="mt-6 text-center"><div className="text-[9px] tracking-[.15em] text-zinc-600">SET {Math.min(reps.length+1,max)} / {max}</div><span className="field-label mt-3">RIPETIZIONI</span><div className="mt-2 flex items-center justify-center gap-2"><button className="counter-btn" disabled={complete} onClick={()=>setInput(String(Math.max(0,(Number(input)||0)-1)))}><Minus size={16}/></button><input aria-label="Reps completed this set" className="counter-input" inputMode="numeric" value={input} onChange={e=>setInput(e.target.value.replace(/\D/g,""))} placeholder="0" disabled={complete}/><button className="counter-btn" disabled={complete} onClick={()=>setInput(String((Number(input)||0)+1))}><Plus size={16}/></button></div>{last!==undefined&&<div className="mt-3 text-[10px] text-zinc-500">LAST SET · {last} reps{diff!==null&&<span className={`ml-2 ${diff>=0?"text-emerald-400":"text-rose-300"}`}>{diff>=0?`+${diff}`:`${diff}`}</span>}</div>}</div><div className="mt-6 grid gap-2 sm:grid-cols-2">{!complete&&!rest&&<button className="primary-cta sm:col-span-2" onClick={saveSet}>SAVE SET</button>}{complete&&!rest&&<button className="primary-cta sm:col-span-2" onClick={()=>finish("complete")}>NEXT EXERCISE</button>}{complete&&rest&&<button className="secondary-cta sm:col-span-2" disabled>REST BEFORE NEXT EXERCISE</button>}{reps.length>0&&!complete&&<button className="secondary-cta sm:col-span-2" onClick={()=>finish("incomplete")}>SALVA INCOMPLETO</button>}</div>{rest&&<Rest seconds={block.rest} sound={sound} vibration={vibration} onSkip={()=>setRest(false)} onDone={()=>setRest(false)}/>}<div className="mt-4 flex flex-wrap justify-center gap-2">{reps.map((r,i)=><button className="chip" key={i} title="Tap to edit this set" onClick={()=>{setEditing(i);setEditValue(String(r))}}>SET {i+1}: {r}</button>)}</div>{editing!==null&&<div className="mt-3 rounded-xl border border-line bg-panel p-3"><span className="field-label">EDIT SET {editing+1}</span><div className="mt-1 flex items-center gap-2"><button className="counter-btn" onClick={()=>setEditValue(String(Math.max(0,Number(editValue||0)-1)))}><Minus size={16}/></button><input className="counter-input flex-1" inputMode="numeric" value={editValue} onChange={e=>setEditValue(e.target.value.replace(/\D/g,""))}/><button className="counter-btn" onClick={()=>setEditValue(String(Number(editValue||0)+1))}><Plus size={16}/></button></div><div className="mt-2 grid grid-cols-2 gap-2"><button className="secondary-cta" onClick={()=>setEditing(null)}>ANNULLA</button><button className="primary-cta" onClick={()=>{const n=Number(editValue);if(Number.isFinite(n)&&n>=0){const next=reps.map((x,j)=>j===editing?n:x);setReps(next);persist(next)}setEditing(null)}}>SAVE EDIT</button></div></div>}<EffortGuide/><div className="mt-auto pb-4 pt-8">{block.bandOptions&&<label className="block mb-3"><span className="field-label">LOOP</span><select value={band} onChange={e=>setBand(e.target.value as Band)}>{block.bandOptions.map(b=><option key={b}>{b}</option>)}</select></label>}<EffortPicker rir={rir} setRir={setRir} fatigue={fatigue} setFatigue={setFatigue}/></div></div>
}
...
function Emom({block,day,onComplete,onStarted,sound,vibration,existing,onProgress}:{block:ExerciseBlock;day:DayKey;onComplete:(l:WorkoutLog)=>void;onStarted:()=>void;sound:boolean;vibration:boolean;existing?:WorkoutLog;onProgress:(l:WorkoutLog)=>void}){
 ...
 return <div className="flex flex-1 flex-col pt-4"><div className="text-center"><div className="text-[9px] tracking-[.16em] text-zinc-600">MINUTE {Math.min(minute,minutes)} / {minutes}</div><div className="mt-2 text-7xl font-extrabold">{phase==="ready"?"01:00":phase==="running"?formatClock(sec):phase==="input"?"INVIO":"FATTO"}</div><div className="mt-4 rounded-2xl border border-line bg-panel p-4 text-left"><div className="grid grid-cols-2 gap-4"><div><span className="field-label">TARGET RANGE / MIN</span><div className="font-bold">{block.target}</div></div><div className="text-right"><span className="field-label">TODAY'S GOAL</span><div className="mt-1 flex justify-end gap-2"><button className="mini-btn" disabled={target<=r.min} onClick={()=>adjust(-1)}><Minus size={14}/></button><b>{target}</b><button className="mini-btn" disabled={target>=r.max} onClick={()=>adjust(1)}><Plus size={14}/></button></div></div></div><div className="mt-4 border-t border-line pt-3"><div className="flex items-center justify-between"><span className="field-label">DURATA</span><span className="text-[9px] text-zinc-600">Recommended {block.minutes||10} min</span></div><div className="mt-2 flex items-center justify-between gap-2"><button className="mini-btn" disabled={minutes<=5||phase!=="ready"} onClick={()=>adjustMinutes(-1)}><Minus size={14}/></button><div className="text-center"><div className="text-2xl font-extrabold">{minutes} min</div><div className="text-[8px] text-zinc-600">5–15 min · before start</div></div><button className="mini-btn" disabled={minutes>=15||phase!=="ready"} onClick={()=>adjustMinutes(1)}><Plus size={14}/></button></div></div></div>{last!==undefined&&<div className="mt-3 text-[10px] text-zinc-500">LAST EXPOSURE · M{minute}: <b className="text-white">{last}</b>{diff!==null&&<span className={`ml-2 ${diff>=0?"text-emerald-400":"text-rose-300"}`}>{diff>=0?`+${diff} vs last`:`${diff} vs last`}</span>}</div>}</div>{(phase==="running"||phase==="input")&&<div className="mt-7 rounded-2xl border border-line bg-panel p-4"><div className="field-label">REPS THIS MINUTE</div><div className="mt-2 flex items-center justify-center gap-2"><button className="counter-btn" disabled={logged} onClick={()=>setInput(String(Math.max(0,(Number(input)||0)-1)))}><Minus size={16}/></button><input aria-label="Reps this minute" className="counter-input w-36" inputMode="numeric" value={input} onChange={e=>setInput(e.target.value.replace(/\D/g,""))} placeholder="0" disabled={logged}/><button className="counter-btn" disabled={logged} onClick={()=>setInput(String((Number(input)||0)+1))}><Plus size={16}/></button></div><button className="primary-cta mt-2 w-full" disabled={logged||input===""} onClick={saveMinute}>{logged?`M${minute} SAVED`:`SAVE M${minute}`}</button></div>}{phase==="ready"&&<button className="primary-cta mt-7 w-full" onClick={start}><Play size={15}/>START {minutes}-MIN EMOM</button>}{canFinish&&<button className="primary-cta mt-5 w-full" onClick={()=>finish("complete")}>SALVA E AVANTI</button>}{vals.length>0&&phase!=="complete"&&<button className="secondary-cta mt-2 w-full" onClick={()=>finish("incomplete")}>SALVA INCOMPLETO</button>}<div className="mt-5 flex flex-wrap justify-center gap-2">{vals.map((v,i)=><button className="chip" key={i} title="Tap to edit this minute" onClick={()=>{setEditing(i);setEditValue(String(v))}}>M{i+1}: {v}</button>)}</div>{editing!==null&&<div className="mt-3 rounded-xl border border-line bg-panel p-3"><span className="field-label">EDIT MINUTE {editing+1}</span><div className="mt-1 flex items-center gap-2"><button className="counter-btn" onClick={()=>setEditValue(String(Math.max(0,Number(editValue||0)-1)))}><Minus size={16}/></button><input className="counter-input flex-1" inputMode="numeric" value={editValue} onChange={e=>setEditValue(e.target.value.replace(/\D/g,""))}/><button className="counter-btn" onClick={()=>setEditValue(String(Number(editValue||0)+1))}><Plus size={16}/></button></div><div className="mt-2 grid grid-cols-2 gap-2"><button className="secondary-cta" onClick={()=>setEditing(null)}>ANNULLA</button><button className="primary-cta" onClick={()=>{const n=Number(editValue);if(n>=0){const next=vals.map((x,j)=>j===editing?n:x);setVals(next);persist(next)}setEditing(null)}}>SAVE EDIT</button></div></div>}{vals.length>0&&<div className="mt-4 rounded-xl border border-line bg-panel p-3 text-center text-[10px] text-muted">TOTAL <b className="text-white">{emomStats(vals).total}</b> · AVG <b className="text-white">{emomStats(vals).avg.toFixed(1)}</b> · DROP-OFF <b className="text-white">{emomStats(vals).drop.toFixed(0)}%</b></div>}<EffortGuide/><div className="mt-auto pb-4 pt-8"><EffortPicker rir={rir} setRir={setRir} fatigue={fatigue} setFatigue={setFatigue}/></div></div>
}
...
function StaticSkill({block,day,onComplete,onStarted,sound,vibration,existing,onProgress}:{block:ExerciseBlock;day:DayKey;onComplete:(l:WorkoutLog)=>void;onStarted:()=>void;sound:boolean;vibration:boolean;existing?:WorkoutLog;onProgress:(l:WorkoutLog)=>void}){
 ...
 return <div className="flex flex-1 flex-col items-center pt-2">
   ...
   <div className={`mt-2 text-[9px] font-extrabold tracking-[.16em] ${phase==="hold"?(status==="BELOW"?"text-rose-300":status==="NEAR"?"text-amber-300":status==="MAX"?"text-emerald-400":"text-violet2"):"text-zinc-600"}`}>{phase==="count"?"GET PRONTO":phase==="hold"?(status==="BELOW"?"BELOW TARGET":status==="NEAR"?"NEAR TARGET":status==="MAX"?"TARGET HIT":"TARGET REACHED"):phase==="stopped"?"ATTEMPT SAVED":"PRONTO"}</div>
   <div className="mt-6 w-full max-w-sm">{phase==="ready"&&<button className="primary-cta w-full" onClick={startCountdown}><Play size={15}/>INIZIA</button>}{phase==="count"&&<div className="text-center text-[10px] text-zinc-500">Get into position.</div>}{phase==="hold"&&<button className="danger-cta w-full" onClick={stop}>FINE SERIE</button>}{phase==="stopped"&&<div className="rounded-2xl border border-line bg-panel p-4"><div className="text-center"><div className="text-3xl font-extrabold">{vals[vals.length-1]?.toFixed(1)}s</div><div className="mt-1 text-[9px] text-zinc-500">{classifyIso(vals[vals.length-1]||0,r.min,r.max)}</div></div><div className="mt-4"><span className="field-label">QUALITÀ</span><div className="mt-2 grid grid-cols-3 gap-2">{(["Clean","Shaky","Lost position"] as const).map(q=><button key={q} onClick={()=>setAttemptQuality(q)} className={`rounded-xl border px-2 py-3 text-[9px] font-bold ${attemptQuality===q?"border-violet-400 bg-violet-500/10 text-violet2":"border-line bg-panel2 text-zinc-500"}`}>{q.toUpperCase()}</button>)}</div></div><div className="mt-4 grid grid-cols-2 gap-2">{vals.length<(block.sets||3)?<button className="secondary-cta" onClick={()=>{stopLock.current=false;setResting(true)}}>REST & NEXT</button>:<button className="secondary-cta" disabled>TUTTO REGISTRATO</button>}{vals.length>=(block.sets||3)?<button className="primary-cta" onClick={finish}>SALVA E AVANTI</button>:<button className="secondary-cta" onClick={()=>onComplete({id:existing?.id||crypto.randomUUID(),sessionId:existing?.sessionId||`draft:${day}`,date:Date.now(),day,exerciseId:block.id,exerciseName:block.name,variantId:getProgramOverride(block.id)?.variantId||getVariant(block.id)?.variantId||block.id,variantName,kind:block.kind,status:"incomplete",prescription:prescriptionSnapshot(block,todayTarget),result:{seconds:vals,band,rir:rir?Number(rir):undefined,fatigue:fatigue?Number(fatigue):undefined,note:"Incomplete static skill"}})}>SALVA INCOMPLETO</button>}</div></div>}</div>
   ...
   {vals.length>0&&<div className="mt-5 w-full max-w-sm rounded-xl border border-line bg-panel2 p-3 text-center"><div className="flex flex-wrap justify-center gap-2">{vals.map((v,i)=><span key={i} className="chip">{v.toFixed(1)}s</span>)}</div>{maxed&&allClean&&<div className="mt-3 text-[9px] font-bold text-emerald-400">PROGRESSION PRONTO</div>}{allInRange&&!maxed&&allClean&&<div className="mt-3 text-[9px] font-bold text-violet2">TARGET COMPLETED</div>}</div>}
   <EffortGuide/><div className="mt-auto w-full max-w-sm pb-[calc(env(safe-area-inset-bottom)+8px)] pt-6">{block.bandOptions&&<label className="block mb-3"><span className="field-label">LOOP</span><select value={band} onChange={e=>setBand(e.target.value as Band)}>{block.bandOptions.map(b=><option key={b}>{b}</option>)}</select></label>}<EffortPicker rir={rir} setRir={setRir} fatigue={fatigue} setFatigue={setFatigue}/></div>
 </div>
}
