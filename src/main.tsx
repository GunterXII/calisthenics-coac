...
function Plan({day,setDay,refresh,remoteCatalog}:{day:DayKey;setDay:(x:DayKey)=>void;refresh:number;remoteCatalog:ExerciseCatalogItem[]}){
 ...
 return <div>
  ...
  <div className="grid gap-2">{p.blocks.map((b,i)=><div key={b.id} className="rounded-2xl border border-line bg-panel p-4">
   <div className="flex items-start justify-between gap-3"><div className="min-w-0"><span className="tag">{LABEL[b.kind]}</span><strong className="mt-1 block">{b.name}</strong><p className="mt-1 text-[10px] text-zinc-500">{b.sets?`${b.sets} sets · `:""}{b.target}{b.rest?` · ${b.rest}s rest`:""}</p>{overrides[b.id]&&<span className="mt-2 inline-block text-[8px] font-bold tracking-[.12em] text-violet2">CUSTOMIZED</span>}</div>{!coachLinked&&<button className="mini-btn shrink-0" aria-label={`Edit ${b.name}`} onClick={()=>{setNotice("");setEditing(b.id)}}>⋯</button>}</div>
...
function ProgramEditor({block,catalog,onClose,onSaved}:{block:ExerciseBlock;catalog:{id:string;name:string;detail?:string;kind:BlockKind;bandOptions?:Band[];category?:string;skill?:string;pattern?:string;difficulty?:number;sideMode?:string;defaultTarget?:string;restSec?:number}[];onClose:()=>void;onSaved:(undo?:()=>void)=>void}){
 ...
 const save=()=>{const s=Math.max(1,Number(sets)||1),r=Math.max(0,Number(rest)||0);const selectedCatalog=catalog.find(x=>x.id===catalogId);const bands=(selectedCatalog?.bandOptions||block.bandOptions||[]).filter(x=>x!=="None") as Band[];const nextName=name.trim()||block.name;const previous=setProgramOverride(block.id,{exerciseId:block.id,catalogExerciseId:catalogId,name:nextName,detail:detail.trim()||undefined,kind,target:target.trim()||block.target,sets:s,rest:r,bandOptions:bands.length?bands:undefined,defaultBand:(band as Band)||undefined,updatedAt:Date.now()});saveCoachDecision({type:"program",exerciseId:block.id,title:`Program changed — ${nextName}`,detail:`Target ${target.trim()||block.target} · ${s} sets · ${r}s rest`,from:block.name,to:nextName});syncProgramLayer().catch(err=>console.warn("Program sync deferred.",err));onSaved(()=>restoreProgramOverride(block.id,previous));};
 const reset=()=>{const previous=getProgramOverride(block.id)||null;clearProgramOverride(block.id);saveCoachDecision({type:"program",exerciseId:block.id,title:`Program reset — ${block.name}`,detail:"Returned to coach default prescription",from:block.name,to:block.name});syncProgramLayer().catch(err=>console.warn("Program sync deferred.",err));onSaved(()=>{if(previous)setProgramOverride(block.id,previous);else clearProgramOverride(block.id);syncProgramLayer().catch(err=>console.warn("Program sync deferred.",err));});};
 return <div className="mt-4 rounded-xl border border-violet-500/15 bg-panel2 p-4">
  ...
  <div className="mt-3 grid grid-cols-3 gap-2"><Field label="SETS" value={sets} set={setSets} placeholder="3"/><Field label="TARGET" value={target} set={setTargetLocal} placeholder="8–10"/><Field label="REST (S)" value={rest} set={setRestLocal} placeholder="90"/></div>
...
function Today({day,setDay,start,draft,onResume}:{day:DayKey;setDay:(x:DayKey)=>void;start:()=>void;draft:any;onResume:()=>void}){
 ...
 return <div>
  ...
  <div className="grid gap-2 md:grid-cols-2">
   ...
   {p.blocks.map((b,i)=><div key={b.id} className="session-row"><div className="session-index">{String(i+1).padStart(2,"0")}</div><div><span className="tag">{LABEL[b.kind]}</span><strong>{currentVariantFor(b.id,b.name)}</strong><p>{b.detail}</p></div></div>)}
  </div>
  <AthleteGoalCard />
...
function AthleteProfileEditor(){
 const [profile,setProfile]=useState<AthleteCoachingProfile>(()=>getLocalAthleteProfile()||{athlete_id:"local",primaryGoal:"",secondaryGoals:[],prioritySkills:[],targetDate:"",notes:"",schedule_days:6,equipment:["Pull-up bar","Dip bars","Loop bands","Parallettes"],preferences:{}});const [busy,setBusy]=useState(false);const [msg,setMsg]=useState("");
 useEffect(()=>{if(supabaseConfigured)fetchMyCoachingProfile().then(p=>{if(p){setProfile(p);saveLocalAthleteProfile(p)}}).catch(()=>{})},[]);
 const save=async()=>{setBusy(true);setMsg("");try{const payload={...profile,secondaryGoals:profile.secondaryGoals||[],prioritySkills:profile.prioritySkills||[],equipment:profile.equipment||[]};if(supabaseConfigured){const saved=await saveMyCoachingProfile(payload);setProfile(saved);saveLocalAthleteProfile(saved)}else saveLocalAthleteProfile(payload);setMsg("PROFILE SAVED");}catch(e:any){setMsg(e?.message||"PROFILE SAVE FAILED")}finally{setBusy(false)}};
 return <div className="mt-8 rounded-2xl border border-line bg-panel p-4"><div className="section-kicker">ATHLETE PROFILE</div><p className="mt-1 text-[9px] text-zinc-600">Your goals and constraints are part of the Coach context.</p><div className="mt-3 grid gap-2 md:grid-cols-2"><label><span className="field-label">PRIMARY GOAL</span><input value={profile.primaryGoal||""} onChange={e=>setProfile({...profile,primaryGoal:e.target.value})} placeholder="Full Front Lever"/></label><label><span className="field-label">TARGET DATE</span><input type="date" value={profile.targetDate||""} onChange={e=>setProfile({...profile,targetDate:e.target.value})}/></label><label><span className="field-label">PRIORITY SKILLS</span><input value={(profile.prioritySkills||[]).join(", ")} onChange={e=>setProfile({...profile,prioritySkills:e.target.value.split(",").map(x=>x.trim()).filter(Boolean)})} placeholder="OAP, Front Lever, Planche"/></label><label><span className="field-label">SECONDARY GOALS</span><input value={(profile.secondaryGoals||[]).join(", ")} onChange={e=>setProfile({...profile,secondaryGoals:e.target.value.split(",").map(x=>x.trim()).filter(Boolean)})}/></label><label><span className="field-label">TRAINING DAYS</span><input inputMode="numeric" value={String(profile.schedule_days??6)} onChange={e=>setProfile({...profile,schedule_days:Number(e.target.value)||0})}/></label><label><span className="field-label">EQUIPMENT</span><input value={(profile.equipment||[]).join(", ")} onChange={e=>setProfile({...profile,equipment:e.target.value.split(",").map(x=>x.trim()).filter(Boolean)})}/></label><label className="md:col-span-2"><span className="field-label">CONTEXT / CONSTRAINTS</span><textarea value={profile.notes||""} onChange={e=>setProfile({...profile,notes:e.target.value})} className="min-h-20 w-full" placeholder="Injuries, preferences, schedule constraints, priorities..."/></label></div><div className="mt-3 flex items-center justify-between"><span className="text-[9px] text-zinc-600">{msg}</span><button className="primary-cta" disabled={busy} onClick={save}>{busy?"SAVING…":"SAVE PROFILE"}</button></div></div>
}
...
function Emom({block,day,onComplete,onStarted,sound,vibration,existing,onProgress}:{block:ExerciseBlock;day:DayKey;onComplete:(l:WorkoutLog)=>void;onStarted:()=>void;sound:boolean;vibration:boolean;existing?:WorkoutLog;onProgress:(l:WorkoutLog)=>void}){
 const minutes=block.minutes||10,r=range(block.target),[vals,setVals]=useState<number[]>(()=>existing?.result.emom||[]),[input,setInput]=useState(""),[phase,setPhase]=useState<"ready"|"running"|"input"|"complete">(()=>existing?.result.emom?.length?"input":"ready"),[minute,setMinute]=useState((existing?.result.emom?.length||0)+1),[endAt,setEndAt]=useState(0),now=useNow(phase==="running"),[logged,setLogged]=useState(false),[target,setTargetLocal]=useState(getTarget(block.id,r.max)??r.max),[rir,setRir]=useState(()=>existing?.result.rir!==undefined?String(existing.result.rir):""),[fatigue,setFatigue]=useState(()=>existing?.result.fatigue?String(existing.result.fatigue):"3"),[editing,setEditing]=useState<number|null>(null),[editValue,setEditValue]=useState("");
 ...
 const persist=(nextVals:number[])=>onProgress({id:existing?.id||crypto.randomUUID(),date:Date.now(),day,exerciseId:block.id,exerciseName:block.name,kind:block.kind,status:"incomplete",result:{emom:nextVals,rir:rir?Number(rir):undefined,fatigue:Number(fatigue),note:`Today target ${target}/min`}});
 ...
 const finish=(status:BlockStatus="complete")=>{setTarget(block.id,target);onComplete({id:existing?.id||crypto.randomUUID(),date:Date.now(),day,exerciseId:block.id,exerciseName:block.name,kind:block.kind,status,result:{emom:vals,rir:rir?Number(rir):undefined,fatigue:Number(fatigue),note:`Today target ${target}/min`}})};
...
function Summary({session,close,onSave}:{session:SessionSummary;close:()=>void;onSave:(s:SessionSummary)=>void}){
 ...
 const handoff=makeCoachHandoff({...session,sessionNote:note}),details=makeSessionReport({...session,sessionNote:note});
 const saveFeedback=()=>{const nextNote=[sessionFeel?`SESSION FEEL: ${sessionFeel}`:"",note.trim()].filter(Boolean).join("\n");const next={...session,sessionNote:nextNote};replaceSession(next);uploadWorkoutSession(next).catch(err=>console.warn("Cloud sync deferred.",err));onSave(next);setSaved(true)};
 const copy=async()=>{try{await navigator.clipboard.writeText(handoff);setCopiedSummary(true);setTimeout(()=>setCopiedSummary(false),1200)}catch{}};
 ...
 return <div className="fixed inset-0 z-50 overflow-y-auto bg-ink text-white"><div className="mx-auto max-w-2xl px-4 py-8 pb-32">
  ...
  <div className="mt-5 rounded-2xl border border-line bg-panel p-5"><div className="section-kicker">COACH SUMMARY</div><pre className="mt-3 whitespace-pre-wrap font-sans text-[10px] leading-5 text-zinc-300">{handoff}</pre><div className="mt-4 grid gap-2 sm:grid-cols-3"><button className="secondary-cta" onClick={copy}>{copiedSummary?"COPIED":"COPY SUMMARY"}</button><button className="secondary-cta" onClick={()=>setShowDetails(v=>!v)}>{showDetails?"HIDE DETAILS":"DETAILS"}</button><button className="secondary-cta" onClick={exportReport}><Download size={14}/>EXPORT</button></div>{showDetails&&<div className="mt-4 rounded-xl border border-line bg-panel2 p-3"><pre className="whitespace-pre-wrap font-sans text-[9px] leading-5 text-zinc-400">{details}</pre></div>}</div>
