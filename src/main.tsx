...
import {evaluateProgression,criteriaForBlock,variantMasteryCriteria,progressionStreak,qualityIsKnown,analyzeReadiness,nextTargetFromSpec,progressionSpecForBlock} from "./coachingEngine";
import "./styles.css";
...
function WorkoutPlayer({state,setState,refresh}:{state:any;setState:(x:any)=>void;refresh:()=>void}){
 ...
 const confirmSkip=()=>{if(!block)return;setSkipAsk(false);next({id:crypto.randomUUID(),sessionId:active.id,date:Date.now(),day:active.day,exerciseId:block.id,exerciseName:block.name,kind:block.kind,status:"skipped",result:{note:"Skipped"}})};
...
function buildSession(a:{id:string;started:number;day:DayKey;readiness:Readiness;sessionNote?:string},logs:WorkoutLog[]):SessionSummary{
 ...
 return{id:sessionId,date:Date.now(),day:a.day,durationSec:Math.round((Date.now()-a.started)/1000),readiness:a.readiness,logs:normalizedLogs,totalReps:normalizedLogs.reduce((s,l)=>s+(l.result.reps?.reduce((x,y)=>x+y,0)||0),0),emomReps:normalizedLogs.reduce((s,l)=>s+(l.result.emom?.reduce((x,y)=>x+y,0)||0),0),bestSkillSeconds:normalizedLogs.filter((l:WorkoutLog)=>l.kind==="SKILL_STATIC").reduce((m,l)=>Math.max(m,Math.max(...(l.result.seconds||[0]))),0)};
}
...
function currentVariantLogs(block:ExerciseBlock){
 ...
 return getLogs().filter(x=>x.exerciseId===block.id&&!x.skipped&&(x.status==="complete"||x.status==="modified")&&logVariantId(x)===exerciseExposureKeyString({exerciseId:block.id,variantId:currentId})).sort((a,b)=>a.date-b.date);
}
...
function SetBlock({block,day,onComplete,onStarted,existing,onProgress,sound,vibration}:{block:ExerciseBlock;day:DayKey;onComplete:(l:WorkoutLog)=>void;onStarted:()=>void;existing?:WorkoutLog;onProgress:(l:WorkoutLog)=>void;sound:boolean;vibration:boolean}){
 const r=range(block.target),[reps,setReps]=useState<number[]>(()=>existing?.result.reps||[]),[input,setInput]=useState(""),[rest,setRest]=useState(false),[band,setBand]=useState<Band>(()=>existing?.result.band||defaultBandFor(block)),[rir,setRir]=useState(()=>existing?.result.rir!==undefined?String(existing.result.rir):""),[fatigue,setFatigue]=useState(()=>existing?.result.fatigue!==undefined?String(existing.result.fatigue):""),[editing,setEditing]=useState<number|null>(null),[editValue,setEditValue]=useState("");
 const variantName=currentVariantFor(block.id,block.name),target=getTarget(block.id,r.max)??r.max,prev=latestLog(day,block.id,existing?.date,getVariant(block.id)?.variantId||block.id),last=prev?.result.reps?.[reps.length],max=block.sets||1,complete=reps.length>=max;
 const persist=(next:number[])=>onProgress({id:existing?.id||crypto.randomUUID(),sessionId:existing?.sessionId||`draft:${day}`,date:Date.now(),day,exerciseId:block.id,exerciseName:block.name,variantId:getVariant(block.id)?.variantId||block.id,variantName,kind:block.kind,status:"incomplete",result:{reps:next,band,rir:rir?Number(rir):undefined,fatigue:fatigue?Number(fatigue):undefined,note:`Today target ${target}/set`}});
...
function Emom({block,day,onComplete,onStarted,sound,vibration,existing,onProgress}:{block:ExerciseBlock;day:DayKey;onComplete:(l:WorkoutLog)=>void;onStarted:()=>void;sound:boolean;vibration:boolean;existing?:WorkoutLog;onProgress:(l:WorkoutLog)=>void}){
 const variantName=currentVariantFor(block.id,block.name),[minutes,setMinutesLocal]=useState(()=>getEmomDuration(block.id,block.minutes||10)),r=range(block.target),[vals,setVals]=useState<number[]>(()=>existing?.result.emom||[]),[input,setInput]=useState(""),[phase,setPhase]=useState<"ready"|"running"|"input"|"complete">(()=>existing?.result.emom?.length?"input":"ready"),[minute,setMinute]=useState((existing?.result.emom?.length||0)+1),[endAt,setEndAt]=useState(0),now=useNow(phase==="running"),[logged,setLogged]=useState(false),[target,setTargetLocal]=useState(getTarget(block.id,r.max)??r.max),[rir,setRir]=useState(()=>existing?.result.rir!==undefined?String(existing.result.rir):""),[fatigue,setFatigue]=useState(()=>existing?.result.fatigue!==undefined?String(existing.result.fatigue):""),[editing,setEditing]=useState<number|null>(null),[editValue,setEditValue]=useState("");
 ...
 const persist=(nextVals:number[])=>onProgress({id:existing?.id||crypto.randomUUID(),sessionId:existing?.sessionId||`draft:${day}`,date:Date.now(),day,exerciseId:block.id,exerciseName:block.name,variantId:getVariant(block.id)?.variantId||block.id,variantName,kind:block.kind,status:"incomplete",result:{emom:nextVals,rir:rir?Number(rir):undefined,fatigue:fatigue?Number(fatigue):undefined,note:`${minutes}-min EMOM · today target ${target}/min`}});
 useEffect(()=>{
 ...
 const finish=(status:BlockStatus="complete")=>{setTarget(block.id,target);onComplete({id:existing?.id||crypto.randomUUID(),sessionId:existing?.sessionId||`draft:${day}`,date:Date.now(),day,exerciseId:block.id,exerciseName:block.name,variantId:getVariant(block.id)?.variantId||block.id,variantName,kind:block.kind,status,result:{emom:vals,rir:rir?Number(rir):undefined,fatigue:fatigue?Number(fatigue):undefined,note:`${minutes}-min EMOM · today target ${target}/min`}})};
