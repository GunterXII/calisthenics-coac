...
function ProgramValidationCard({refresh}:{refresh:number}){
 ...
 return <div className="mb-5 rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4">
  ...
  <div className="mt-4 grid gap-2 sm:grid-cols-3"><div className="rounded-xl border border-line bg-panel p-3"><div className="field-label">ADJUSTED SETS</div><div className="mt-1 text-lg font-extrabold">{totalSets.toFixed(1)}</div></div><div className="rounded-xl border border-line bg-panel p-3"><div className="field-label">FATIGUE LOAD</div><div className="mt-1 text-lg font-extrabold">{totalFatigue.toFixed(1)}</div></div><div className="rounded-xl border border-line bg-panel p-3"><div className="field-label">GRIP DEMAND</div><div className="mt-1 text-lg font-extrabold">{totalGrip.toFixed(1)}</div></div></div>
  ...
  {report.signals.length>0&&<button className="secondary-cta mt-3 w-full" onClick={()=>{setExpanded(v=>!v);track("program_validation_toggled",{expanded:!expanded,signals:report.signals.length})}}>{expanded?"SHOW FEWER SIGNALS":`SHOW ALL SIGNALS · ${report.signals.length}`}</button>}
...
function Nav({tab,setTab}:{tab:string;setTab:(x:any)=>void}){
 const items:[[string,string,any],[string,string,any],[string,string,any],[string,string,any]]=[["today","TODAY",CalendarDays],["plan","PLAN",ListChecks],["reports","REPORTS",BarChart3],["settings","SETTINGS",Settings2]];
 return <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-line bg-ink/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl" aria-label="Primary navigation"><div className="mx-auto grid h-[72px] max-w-5xl grid-cols-4 px-2 sm:h-[76px]">{items.map(([id,l,Icon])=><button key={id} aria-label={l} aria-current={tab===id?"page":undefined} onClick={()=>{track("navigation_clicked",{destination:id});setTab(id)}} className={`nav-item ${tab===id?"nav-item-active":""}`}><Icon size={18}/><span>{l}</span></button>)}</div></nav>
}
...
function CoachVerdict({session}:{session:SessionSummary}){
 ...
 return <section className="mt-4 rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4"><div className="flex items-start justify-between gap-3"><div><div className="section-kicker">COACH VERDICT</div><div className={`mt-1 text-lg font-extrabold ${tone}`}>{label}</div><p className="mt-2 text-[10px] leading-5 text-zinc-400">{strong.detail}</p></div><span className="tag">POST-SESSION</span></div><div className="mt-3 grid gap-2 sm:grid-cols-3">{strong.reasons.map((x:string)=><div key={x} className="rounded-xl border border-line bg-panel p-3 text-[9px] leading-4 text-zinc-500">{x}</div>)}</div>{decisions.length>0&&<div className="mt-4 rounded-xl border border-line bg-panel p-3"><div className="flex items-end justify-between gap-3"><div><div className="section-kicker">EXERCISE DECISIONS</div><div className="mt-1 text-[9px] text-zinc-600">Performance + same-prescription history + readiness + recovery.</div></div><span className="tag">COACH 2.0</span></div><div className="mt-3 grid gap-2">{decisions.slice(0,8).map(d=><div key={d.context.current.id} className="rounded-xl border border-line bg-panel2 p-3"><div className="flex items-start justify-between gap-3"><div><div className="text-sm font-extrabold">{d.context.block.name}</div><div className="mt-1 text-[8px] text-zinc-600">{d.performanceBand} · {d.context.readinessStatus} · {d.context.comparableExposures} comparable</div></div><span className={`text-[8px] font-extrabold tracking-[.12em] ${dTone(d.decision)}`}>{d.decision}</span></div><div className="mt-2 text-[9px] leading-4 text-zinc-500">{d.reasons[0]}</div>{d.progressionStreak>0&&<div className="mt-2 text-[8px] font-bold tracking-[.1em] text-violet2">QUALIFYING EXPOSURES · {d.progressionStreak}</div>}</div>)}</div></div>}<div className="mt-4 rounded-xl border border-line bg-panel p-3"><div className="flex items-center justify-between gap-3"><div><div className="section-kicker">WORKLOAD & RECOVERY</div><div className="mt-1 text-[9px] text-zinc-600">7-day planning heuristic · not a physiological measurement</div></div><span className={`text-[8px] font-extrabold tracking-[.12em] ${recTone(recovery.report.overallRecovery)}`}>{recovery.report.overallRecovery}</span></div><div className="mt-3 grid grid-cols-3 gap-2"><div><div className="field-label">ADJUSTED SETS</div><div className="mt-1 text-sm font-extrabold">{recovery.report.totalAdjustedSets.toFixed(1)}</div></div><div><div className="field-label">FATIGUE LOAD</div><div className="mt-1 text-sm font-extrabold">{recovery.report.totalFatigueLoad.toFixed(1)}</div></div><div><div className="field-label">GRIP</div><div className="mt-1 text-sm font-extrabold">{recovery.report.grip.score.toFixed(1)}</div></div></div>{muscleRows.length>0&&<div className="mt-3 grid gap-2">{muscleRows.map(m=><div key={m.muscle} className="flex items-center justify-between rounded-lg bg-panel2 px-3 py-2"><span className="text-[9px] font-bold text-zinc-400">{pretty(m.muscle)}</span><span className="text-[9px] text-zinc-600">{m.adjustedSets.toFixed(1)} sets · {Math.round(recovery.report.recovery[m.muscle]?.recoveryPct||0)}% recovered</span></div>)}</div>}{recovery.flags.length>0&&<div className="mt-3 text-[9px] leading-4 text-amber-300">{recovery.flags.slice(0,3).join(" · ")}</div>}</div></section>
}
...
function AnalyticsPanel(){
 ...
 return <div className="mt-8 rounded-2xl border border-line bg-panel p-4">
  ...
  <div className="mt-4 grid gap-3 lg:grid-cols-2">
   ...
   <div className="rounded-xl border border-line bg-panel2 p-3"><div className="section-kicker">FEATURE ADOPTION</div><div className="mt-3 grid gap-1">{intel.featureAdoption.slice(0,8).map(([name,count])=><div key={name} className="flex items-center justify-between text-[9px]"><span className="text-zinc-400">{name}</span><b>{count}</b></div>)}</div></div>
  </div>
  <div className="mt-3 grid gap-3 lg:grid-cols-2">
   <div className="rounded-xl border border-line bg-panel2 p-3"><div className="section-kicker">EXERCISE FRICTION</div><p className="mt-1 text-[8px] text-zinc-600">Started → skipped/substituted. High friction is a signal to review UX or exercise selection.</p>{riskExercises.length?<div className="mt-3 grid gap-2">{riskExercises.map(x=><div key={x.exerciseId} className="rounded-lg border border-line p-2"><div className="flex items-center justify-between text-[9px]"><b>{x.exerciseId}</b><span className="text-amber-300">{x.frictionRate.toFixed(0)}% friction</span></div><div className="mt-1 text-[8px] text-zinc-600">started {x.started} · complete {x.completed} · skipped {x.skipped} · substituted {x.substituted}</div></div>)}</div>:<div className="mt-3 text-[9px] text-zinc-600">No high-friction exercises in this window.</div>}</div>
   <div className="rounded-xl border border-line bg-panel2 p-3"><div className="section-kicker">RECOVERY / ABANDONMENT SIGNALS</div><div className="mt-3 grid gap-1 text-[9px]"><div className="flex justify-between"><span className="text-zinc-400">Resumed workouts</span><b>{intel.recovery.resumed}</b></div><div className="flex justify-between"><span className="text-zinc-400">Saved drafts</span><b>{intel.recovery.draftSaved}</b></div><div className="flex justify-between"><span className="text-zinc-400">Discarded workouts</span><b>{intel.recovery.discarded}</b></div><div className="flex justify-between"><span className="text-zinc-400">Timer skips</span><b>{intel.timer.skips}</b></div><div className="flex justify-between"><span className="text-zinc-400">Program edits</span><b>{intel.program.edits}</b></div></div></div>
  </div>
  <div className="mt-3 rounded-xl border border-line bg-panel2 p-3"><div className="section-kicker">TOP SCREENS</div><div className="mt-2 grid gap-1 sm:grid-cols-2">{intel.topScreens.map(([name,count])=><div key={name} className="flex items-center justify-between text-[9px]"><span className="text-zinc-400">{name}</span><b>{count}</b></div>)}</div></div>
  <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3"><button className="secondary-cta" onClick={()=>{track("analytics_exported");doExport()}}>EXPORT</button><button className="secondary-cta" onClick={()=>{clear();track("analytics_cleared")}}>CLEAR</button><div className="flex items-center justify-center text-[8px] text-zinc-600">Window: {days} days</div></div>
 </div>
}
function Settings(){return <div><div className="eyebrow">SETTINGS</div><h1>App settings.</h1><p className="sub">Profile, coach connection and local data stay outside performance reports.</p><CoachLinkCard onLinked={()=>track("coach_linked")}/><AthleteProfileEditor/><DataBackup/><AnalyticsPanel/></div>}
...
function buildSession(a:{id:string;started:number;day:DayKey;readiness:Readiness;sessionNote?:string},logs:WorkoutLog[]):SessionSummary{
...
function PromotionPanel({block,coachLinked=false}:{block:ExerciseBlock;coachLinked?:boolean}){
  ...
  return <div className="my-3 rounded-2xl border border-line bg-panel p-4">
    <div className="flex items-start justify-between gap-3"><div><span className="field-label">CURRENT VARIANT</span><div className="mt-1 text-sm font-bold">{currentName}</div></div><span className="tag">COACH REVIEW ONLY</span></div>
...
function StaticSkill({block,day,onComplete,onStarted,sound,vibration,existing,onProgress}:{block:ExerciseBlock;day:DayKey;onComplete:(l:WorkoutLog)=>void;onStarted:()=>void;sound:boolean;vibration:boolean;existing?:WorkoutLog;onProgress:(l:WorkoutLog)=>void}){
 ...
 return <div className="flex flex-1 flex-col items-center pt-2">
   ...
   <div className="mt-6 w-full max-w-sm">{phase==="ready"&&<button className="primary-cta w-full" onClick={startCountdown}><Play size={15}/>START</button>}{phase==="count"&&<div className="text-center text-[10px] text-zinc-500">Get into position.</div>}{phase==="hold"&&<button className="danger-cta w-full" onClick={stop}>END SET</button>}{phase==="stopped"&&<div className="rounded-2xl border border-line bg-panel p-4"><div className="text-center"><div className="text-3xl font-extrabold">{vals[vals.length-1]?.toFixed(1)}s</div><div className="mt-1 text-[9px] text-zinc-500">{classifyIso(vals[vals.length-1]||0,r.min,r.max)}</div></div><div className="mt-4"><span className="field-label">QUALITY</span><div className="mt-2 grid grid-cols-3 gap-2">{(["Clean","Shaky","Lost position"] as const).map(q=><button key={q} onClick={()=>setAttemptQuality(q)} className={`rounded-xl border px-2 py-3 text-[9px] font-bold ${attemptQuality===q?"border-violet-400 bg-violet-500/10 text-violet2":"border-line bg-panel2 text-zinc-500"}`}>{q.toUpperCase()}</button>)}</div></div><div className="mt-4 grid grid-cols-2 gap-2">{vals.length<(block.sets||3)?<button className="secondary-cta" onClick={()=>{stopLock.current=false;setResting(true)}}>REST & NEXT</button>:<button className="secondary-cta" disabled>ALL LOGGED</button>}{vals.length>=(block.sets||3)?<button className="primary-cta" onClick={finish}>SAVE & NEXT</button>:<button className="secondary-cta" onClick={()=>onComplete({id:existing?.id||crypto.randomUUID(),sessionId:existing?.sessionId||`draft:${day}`,date:Date.now(),day,exerciseId:block.id,exerciseName:block.name,variantId:getProgramOverride(block.id)?.variantId||getVariant(block.id)?.variantId||block.id,variantName,kind:block.kind,status:"incomplete",prescription:prescriptionSnapshot(block,todayTarget),result:{seconds:vals,band,rir:rir?Number(rir):undefined,fatigue:fatigue?Number(fatigue):undefined,note:"Incomplete static skill"}})}>SAVE INCOMPLETE</button>}</div></div>}</div>
...
function CoachProposalPanel({session}:{session:SessionSummary}){
 ...
 const apply=async(p:CoachProposal,accept:boolean)=>{
   ...
   if(!block){setMessage("Exercise is no longer present in the active plan.");return}
   ...
   acceptCoachProposalAtomically(p.id,override,variantState,{type:p.type==="variant"?"progression":"program",exerciseId:p.exerciseId,title:`Proposal accepted — ${p.title}`,detail:`${p.reason} Applied to future sessions.`,from:p.from,to:p.to});
