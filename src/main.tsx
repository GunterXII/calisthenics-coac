...
function Plan({day,setDay,refresh,remoteCatalog}:{day:DayKey;setDay:(x:DayKey)=>void;refresh:number;remoteCatalog:ExerciseCatalogItem[]}){
 ...
 return <div>
  <div className="eyebrow">PIANO</div><div className="flex items-end justify-between gap-4"><div><h1>Il tuo programma.</h1><p className="sub">{coachLinked?"Programma gestito dal Coach.":"Il tuo programma di allenamento."}</p></div>{!coachLinked&&<button className="secondary-cta" onClick={()=>{const snapshot=getProgramOverrides();clearAllProgramOverrides();syncProgramLayer().catch(err=>console.warn("Program sync deferred.",err));showNotice("PROGRAMMA RIPRISTINATO AI DEFAULT DEL COACH",()=>{Object.values(snapshot).forEach((o:any)=>setProgramOverride(o.exerciseId,o));syncProgramLayer().catch(err=>console.warn("Program sync deferred.",err));})}}>RIPRISTINA TUTTO</button>}</div>
  ...
  <div className="mb-4 rounded-2xl border border-violet-500/15 bg-violet-500/5 p-4"><div className="flex items-start justify-between gap-3"><div><div className="section-kicker">COSA CAMBIA QUESTA SETTIMANA</div><p className="mt-1 text-[9px] leading-4 text-zinc-500">Qui vedi solo ciò che il Coach ha modificato rispetto al piano base. Una riduzione di serie o minuti significa che il volume viene abbassato temporaneamente per proteggere recupero e qualità dello stimolo.</p></div><span className="tag">{phaseLabel(currentPhase(Date.now()).type)}</span></div>{weeklyCoachChanges(day).length?<div className="mt-3 grid gap-2 sm:grid-cols-2">{weeklyCoachChanges(day).map(x=><div key={x.id} className="rounded-xl border border-line bg-panel p-3"><div className="text-[10px] font-extrabold">{x.name}</div><div className="mt-1 text-[9px] text-zinc-500">{x.before} → <b className="text-white">{x.after}</b></div>{/\b(min|serie|sets)\b/i.test(String(x.before)) && /\b(min|serie|sets)\b/i.test(String(x.after)) && Number(String(x.after).match(/\d+(?:\.\d+)?/)?.[0]||0)<Number(String(x.before).match(/\d+(?:\.\d+)?/)?.[0]||0)&&<div className="mt-1 text-[8px] font-bold tracking-[.08em] text-amber-300">VOLUME RIDOTTO · PRIORITÀ AL RECUPERO</div>}</div>)}</div>:<div className="mt-3 text-[9px] text-zinc-600">Nessuna modifica rispetto al piano di riferimento.</div>}</div>
...
function ProgramValidationCard({refresh}:{refresh:number}){
 ...
 return <div className="mb-5 rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4">
  <div className="flex items-start justify-between gap-3"><div><div className="section-kicker">CONTROLLO PROGRAMMA</div><div className="mt-1 text-sm font-extrabold">Simulazione settimanale</div><p className="mt-1 text-[10px] leading-5 text-zinc-500">Controlla carico previsto, distanza tra gli stimoli e presa demand, overlap and sequencing before you run the week.</p></div><div className="text-right"><div className={`text-lg font-extrabold ${scoreTone}`}>{report.score}</div><div className="text-[8px] font-bold tracking-[.12em] text-zinc-600">{report.severity}</div></div></div>
...
function CoachLinkCard({onLinked}:{onLinked:()=>void}){
 ...
 return <div className="mt-5 rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4"><div className="section-kicker">COLLEGAMENTO COACH</div>{coach?<div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-sm font-extrabold">{coach.display_name||'Il tuo Coach'}</div><div className="mt-1 text-[9px] text-zinc-600">COLLEGATO {new Date(coach.linked_at).toLocaleDateString()}</div></div><button className="secondary-cta" disabled={busy} onClick={unlink}>{busy?'IN CORSO…':'SCOLLEGA COACH'}</button></div>:<><p className="mt-2 text-[10px] leading-5 text-zinc-500">Chiedi al tuo Coach di inviarti il codice di 10 caratteri. Linking does not change your workout history.</p><div className="mt-3 flex gap-2"><input className="flex-1 rounded-xl border border-line bg-panel2 p-3 font-mono text-sm uppercase tracking-[.18em]" maxLength={10} value={code} onChange={e=>setCode(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g,''))} placeholder="CODICE COACH"/><button className="primary-cta" disabled={busy||code.length!==10} onClick={link}>{busy?'LINKING…':'LINK'}</button></div></>}{msg&&<div className={`mt-2 text-[9px] ${msg.includes('FAILED')?'text-rose-300':'text-emerald-300'}`}>{msg}</div>}</div>;
}
...
function SkillIntelligencePanel({insights}:{insights:SkillInsight[]}){
 ...
 return <div className="mt-5 rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4">
  <div className="flex items-end justify-between gap-3"><div><div className="section-kicker">SKILL INTELLIGENCE</div><div className="mt-1 text-[9px] text-zinc-500">Motore di progressione basato sui dati: qualificazioni consecutive, qualità, recupero e fatica.</div></div><span className="tag">MOTORE COACH</span></div>
...
function SkillGraphPanel({catalog,blocks,insights,onPromote}:{catalog:ExerciseCatalogItem[];blocks:ExerciseBlock[];insights:SkillInsight[];onPromote:(view:SkillGraphView)=>void}){
 ...
 return <div className="mt-5 rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4">
  <div className="flex items-end justify-between gap-3"><div><div className="section-kicker">SKILL GRAPH</div><div className="mt-1 text-[9px] text-zinc-500">Percorso meccanico + verifica sui dati. PRONTO significa che il livello attuale ha superato i criteri; la promozione richiede comunque la conferma del Coach.</div></div><span className="tag">PERCORSI</span></div>
 ...
 return <div>
   ...
   {!athletes.length?<div className="mt-6 grid gap-3 md:grid-cols-[1.2fr_.8fr]"><div className="rounded-2xl border border-line bg-panel p-5"><div className="section-kicker">I TUOI ATLETI</div><h2 className="mt-2 text-2xl font-extrabold tracking-tight">No athletes yet.</h2><p className="mt-2 max-w-xl text-xs leading-5 text-zinc-500">Give your 10-character Coach Code to an athlete. Once they connect from Reports → Coach, their training evidence will appear here automatically.</p><div className="mt-5 rounded-xl border border-line bg-panel2 p-4"><div className="field-label">NEXT STEP</div><div className="mt-2 text-sm font-bold">Wait for the first athlete connection.</div><div className="mt-1 text-[9px] leading-4 text-zinc-600">The Coach does not need to create athletes manually.</div></div></div><div className="rounded-2xl border border-violet-500/20 bg-violet-500/5 p-5"><div className="section-kicker">COACHING LOOP</div><div className="mt-3 grid gap-2 text-[10px] font-bold"><div className="rounded-xl bg-panel2 p-3">01 · ATHLETE TRAINS</div><div className="rounded-xl bg-panel2 p-3">02 · EVIDENCE SYNCs</div><div className="rounded-xl bg-panel2 p-3">03 · COACH REVIEWS</div><div className="rounded-xl bg-panel2 p-3">04 · COACH PUBLISHES</div></div></div></div>:<div className="mt-6 grid gap-3 md:grid-cols-[240px_1fr]">
    ...
    <div className="min-w-0">
      ...
      {athlete&&<div className="mt-3 grid gap-2 md:grid-cols-3"><div className="rounded-2xl border border-line bg-panel p-4"><div className="section-kicker">READINESS</div><div className="mt-2 text-sm font-extrabold">{latestReadiness.energy!==undefined?`${latestReadiness.energy}/5 energy`:"Nessun recupero registrato"}</div><div className="mt-1 text-[9px] text-zinc-600">{latestReadiness.sleepHours!==undefined?`${latestReadiness.sleepHours}h sleep`:"Sonno non registrato"}</div></div><div className="rounded-2xl border border-line bg-panel p-4"><div className="section-kicker">FLAGS</div><div className="mt-2 text-sm font-extrabold">{flags.length?flags.join(" · "):"NESSUN AVVISO ATTIVO"}</div><div className="mt-1 text-[9px] text-zinc-600">Review signals only.</div></div><div className="rounded-2xl border border-line bg-panel p-4"><div className="section-kicker">LATEST NOTE</div><div className="mt-2 text-sm font-extrabold line-clamp-2">{sessions[0]?.session_note||"No session note yet."}</div></div></div>}
...
function Today({day,setDay,start,draft,onResume}:{day:DayKey;setDay:(x:DayKey)=>void;start:()=>void;draft:any;onResume:()=>void}){
 ...
 return <div>
  ...
  <div className="mt-4 grid gap-2 md:grid-cols-2">
    {last?<div className="rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4"><div className="section-kicker">ULTIMA SESSIONE</div><div className="mt-2 flex flex-wrap gap-3 text-[10px] text-zinc-400"><span>{Math.round(last.durationSec/60)} min</span><span>{last.totalReps} reps</span><span>{last.emomReps} EMOM</span></div></div>:<div className="rounded-2xl border border-line bg-panel p-4"><div className="section-kicker">PRIMA SESSIONE</div><p className="mt-2 text-[10px] leading-5 text-zinc-500">La tua cronologia di performance apparirà qui dopo la prima sessione completata.</p></div>}
...
function WorkoutPlayer({state,setState,refresh}:{state:any;setState:(x:any)=>void;refresh:()=>void}){
 ...
 return <div className="fixed inset-0 z-50 flex min-h-[100dvh] flex-col bg-ink text-white">
  ...
  {exitAsk&&<div className="fixed inset-0 z-[70] flex items-end bg-black/75 pb-[env(safe-area-inset-bottom)] backdrop-blur-sm"><div className="mx-auto w-full max-w-xl rounded-t-3xl border border-line bg-panel p-5"><div className="eyebrow">ABBANDONARE L’ALLENAMENTO?</div><h2 className="mt-2 text-2xl font-extrabold">Questa sessione non è terminata.</h2><p className="mt-2 text-xs text-muted">Riprendila più tardi, continua ad allenarti oppure elimina la bozza.</p><div className="mt-5 grid gap-2 sm:grid-cols-3"><button className="secondary-cta" onClick={()=>setExitAsk(false)}>KEEP TRAINING</button><button className="secondary-cta" onClick={()=>{track("workout_draft_saved",{day:active.day,index:active.index});saveDraft(active);setState(null)}}>SAVE DRAFT & EXIT</button><button className="danger-cta" onClick={()=>{track("workout_discarded",{day:active.day,index:active.index});clearDraft();setState(null)}}>ELIMINA</button></div></div></div>}
...
function StaticSkill({block,day,onComplete,onStarted,sound,vibration,existing,onProgress}:{block:ExerciseBlock;day:DayKey;onComplete:(l:WorkoutLog)=>void;onStarted:()=>void;sound:boolean;vibration:boolean;existing?:WorkoutLog;onProgress:(l:WorkoutLog)=>void}){
 ...
 return <div className="flex flex-1 flex-col items-center pt-2">
   ...
   <div className={`mt-2 text-[9px] font-extrabold tracking-[.16em] ${phase==="hold"?(status==="BELOW"?"text-rose-300":status==="NEAR"?"text-amber-300":status==="MAX"?"text-emerald-400":"text-violet2"):"text-zinc-600"}`}>{phase==="count"?"GET PRONTO":phase==="hold"?(status==="BELOW"?"BELOW TARGET":status==="NEAR"?"NEAR TARGET":status==="MAX"?"TARGET HIT":"TARGET RAGGIUNTO"):phase==="stopped"?"ATTEMPT SAVED":"PRONTO"}</div>
