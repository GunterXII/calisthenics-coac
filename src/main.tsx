...
function feedback(kind:"countdown"|"start"|"complete"|"rest",sound:boolean,vibration:boolean){
  ...
  if(vibration){try{const pattern=kind==="complete"?[70,45,100]:kind==="start"?[45,30,55]:kind==="countdown"?35:70;navigator.vibrate?.(pattern)}catch{}}
}
...
function Plan({day,setDay,refresh,remoteCatalog}:{day:DayKey;setDay:(x:DayKey)=>void;refresh:number;remoteCatalog:ExerciseCatalogItem[]}){
 ...
 return <div className="plan-screen">
  ...
  <div className="mt-5 plan-validation-wrap"><ProgramValidationCard refresh={refresh}/></div>
  <div className="plan-day-header mt-7"><div><div className="section-kicker">{p.title}</div><h2 className="mt-1 text-2xl font-extrabold tracking-tight">{uiCopy(p.subtitle)}</h2><p className="plan-day-copy">La prescrizione attuale, esercizio per esercizio.</p></div><div className="plan-count"><strong>{p.blocks.length}</strong><span>ESERCIZI</span></div></div>
  ...
  <div className="mt-3 grid gap-2">{p.blocks.map((b,i)=><div key={b.id} className={`plan-exercise-row rounded-2xl border border-line bg-panel p-4 ${overrides[b.id]?"is-customized":""}`}>
   <div className="flex items-start gap-3"><div className="plan-exercise-number">{String(i+1).padStart(2,"0")}</div><div className="min-w-0 flex-1"><div className="plan-exercise-head"><div className="min-w-0"><span className="tag">{LABEL[b.kind]}</span><strong className="mt-1 block text-[15px] leading-tight">{currentVariantFor(b.id,b.name)}</strong></div>{!coachLinked&&<button className="mini-btn shrink-0" aria-label={`Modifica ${b.name}`} onClick={()=>{track("program_edit_opened",{exerciseId:b.id,day});setNotice("");setEditing(b.id)}}>⋯</button>}</div>
    <div className="plan-prescription-row"><span className="prescription-chip">{b.kind==="EMOM"?`${b.minutes||10} MIN`:b.sets?`${b.sets} × ${b.target}`:b.target}</span>{b.rest? <span className="prescription-chip">{b.rest}s REC</span>:null}{overrides[b.id]&&<span className="prescription-chip prescription-chip-accent">PERSONALIZZATO</span>}</div>
    {b.detail&&<p className="plan-exercise-detail">{displayBlockDetail(b,uiCopy)}</p>}{coachNoteForBlock(b,uiCopy)&&<div className="plan-coach-note"><span>COACH · </span>{coachNoteForBlock(b,uiCopy)}</div>}
   </div></div>
...
function ProgramEditor({block,catalog,onClose,onSaved}:{block:ExerciseBlock;catalog:{id:string;name:string;detail?:string;kind:BlockKind;bandOptions?:Band[];category?:string;skill?:string;pattern?:string;difficulty?:number;sideMode?:string;defaultTarget?:string;restSec?:number}[];onClose:()=>void;onSaved:(undo?:()=>void)=>void}){
 ...
 return <div className="mt-4 rounded-xl border border-accent bg-panel2 p-4">
...
function CoachTimeline({sessions,program}:{sessions:any[];program:any}){const rows=coachTimelineRows(sessions,program);return <div className="mt-5 rounded-2xl border border-accent bg-accent-soft p-4"><div className="flex items-end justify-between gap-3"><div><div className="section-kicker">STORICO ATLETA</div><div className="mt-1 text-[9px] text-zinc-500">Ultime esposizioni, miglior risultato e direzione recente.</div></div><span className="tag">ULTIME 6</span></div>{!rows.length?<div className="mt-3 text-xs text-zinc-600">Non ci sono ancora abbastanza dati sugli esercizi.</div>:<div className="mt-3 grid gap-2 lg:grid-cols-2">{rows.map((r:any)=><div key={r.id} className="rounded-xl border border-line bg-panel p-3"><div className="flex items-start justify-between gap-3"><div><div className="text-[8px] text-zinc-600">{r.exposures} EXPOSURES</div><div className="mt-1 text-sm font-extrabold">{r.name}</div></div><span className={`text-[8px] font-extrabold tracking-[.12em] ${r.signal==="PROGRESSING"?"text-emerald-400":r.signal==="REGRESSING"?"text-amber-300":"text-accent"}`}>{r.signal}</span></div><div className="mt-3 grid grid-cols-3 gap-2"><div><div className="field-label">ULTIMO</div><div className="mt-1 text-sm font-bold">{r.latest.toFixed(r.unit==="s"?1:0)} <span className="text-[8px] text-zinc-600">{r.unit}</span></div></div><div><div className="field-label">MIGLIORE</div><div className="mt-1 text-sm font-bold">{r.best.toFixed(r.unit==="s"?1:0)} <span className="text-[8px] text-zinc-600">{r.unit}</span></div></div><div><div className="field-label">VARIAZIONE</div><div className="mt-1 text-sm font-bold">{r.exposures>1?`${r.delta>=0?"+":""}${r.delta.toFixed(0)}%`:"—"}</div></div></div></div>)}</div>}</div>}

function SkillIntelligencePanel({insights}:{insights:SkillInsight[]}){
 const color=(d:SkillInsight["decision"])=>d==="PROGRESS"?"text-emerald-400":d==="REGRESS"?"text-rose-400":d==="REVIEW"?"text-amber-300":d==="HOLD"?"text-accent":"text-zinc-500";
 return <div className="mt-5 rounded-2xl border border-accent bg-accent-soft p-4">
  ...
  {!insights.length?<div className="mt-3 text-xs text-zinc-600">Non ci sono ancora abbastanza dati sulle skill.</div>:<div className="mt-3 grid gap-2 lg:grid-cols-2">
   {insights.slice(0,10).map(x=><div key={x.id} className="rounded-xl border border-line bg-panel p-3">
    ...
    <div className="mt-2 flex items-center justify-between gap-2"><span className="text-[8px] font-bold tracking-[.1em] text-accent">{x.action}</span>{x.next&&<span className="text-right text-[8px] text-zinc-600">NEXT · {x.next}</span>}</div>
   </div>)}
  </div>}
 </div>;
}
...
function SkillGraphPanel({catalog,blocks,insights,onPromote}:{catalog:ExerciseCatalogItem[];blocks:ExerciseBlock[];insights:SkillInsight[];onPromote:(view:SkillGraphView)=>void}){
 ...
 const tone=(s:string)=>s==="PRONTO"?"text-emerald-400":s==="ATTUALE"?"text-accent":s==="LOCKED"?"text-amber-300":"text-zinc-600";
 return <div className="mt-5 rounded-2xl border border-accent bg-accent-soft p-4">
  ...
  <div className="mt-3 grid gap-2 lg:grid-cols-2">{views.map(v=><div key={v.id} className="rounded-xl border border-line bg-panel p-3">
   ...
   <div className="mt-3 flex items-center gap-1 overflow-x-auto pb-1">{v.nodes.map((n,i)=><div key={n.id} className={`min-w-[92px] rounded-lg border px-2 py-2 ${i===v.currentIndex?"border-accent-strong bg-accent-soft":"border-line bg-panel2"}`}><div className="text-[8px] font-bold">{i+1}. {n.name}</div><div className="mt-1 text-[7px] text-zinc-600">D{n.difficulty}</div></div>)}</div>
   <div className="mt-3 rounded-lg bg-panel2 p-2 text-[9px] leading-4 text-zinc-500">{v.note}</div>
   {v.next&&<div className="mt-2 flex items-center justify-between gap-2"><div><span className="text-[8px] font-bold tracking-[.1em] text-accent">PROSSIMO LIVELLO</span><div className="text-[9px] font-bold">{v.next.name}</div></div>{v.status==="READY"&&<button className="primary-cta shrink-0 !py-2" onClick={()=>onPromote(v)}>REVISIONA PROMOZIONE</button>}</div>}
  </div>)}</div>
 </div>;
}
   ...
   {!rows.length?<div className="mt-3 text-xs text-zinc-600">Nessuna decisione da revisionare.</div>:<div className="mt-3 grid gap-2 lg:grid-cols-2">
    {rows.slice(0,8).map((r:any)=>{
      ...
      return <div key={r.id} className="rounded-xl border border-line bg-panel p-3">
       ...
       <div className="mt-2 rounded-lg bg-panel2 p-2 text-[8px] font-bold tracking-[.1em] text-accent">RACCOMANDAZIONE · {r.action}</div>
...
function CoachPerformanceDashboard({sessions,program,decisions}:{sessions:any[];program:any;decisions:any[]}){
 ...
 const status=(r:any)=>r.signal==="PROGRESSING"?"text-emerald-400":r.signal==="REGRESSING"?"text-amber-300":"text-accent";
 return <div className="mt-5 rounded-2xl border border-accent bg-accent-soft p-4">
...
function CoachWorkspace({remoteCatalog}:{remoteCatalog:ExerciseCatalogItem[]}){
 ...
 return <div>
   ...
   <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><div className="eyebrow">SPAZIO COACH</div><h1>Gestisci gli atleti.</h1><p className="sub">Un unico spazio per leggere i dati, prescrivere il lavoro e registrare le decisioni.</p></div><div className="rounded-2xl border border-accent bg-accent-soft px-4 py-3 md:min-w-[220px]"><div className="flex items-center justify-between gap-4"><div className="field-label !mb-0">CODICE COACH</div><span className="text-[8px] font-bold tracking-[.12em] text-emerald-400">SOLO COLLEGAMENTO</span></div><div className="mt-1 flex items-center justify-between gap-3"><div className="font-mono text-lg font-extrabold tracking-[.18em]">{coachCode||"—"}</div><button className="secondary-cta !px-3 !py-2" disabled={!coachCode} onClick={copyCoachCode}>COPIA</button></div><div className="mt-2 text-[8px] leading-4 text-zinc-600">Share this code with an athlete. It does not grant access by itself.</div></div></div><div className="mt-4 sticky top-[70px] z-20 rounded-2xl border border-line bg-ink/95 p-2 backdrop-blur-xl"><div className="grid grid-cols-3 gap-1"><button className="secondary-cta !py-2" onClick={()=>document.getElementById("coach-intelligence")?.scrollIntoView({behavior:"smooth",block:"start"})}>OVERVEDI</button><button className="secondary-cta !py-2" onClick={()=>document.getElementById("coach-program")?.scrollIntoView({behavior:"smooth",block:"start"})}>PROGRAMMA</button><button className="secondary-cta !py-2" onClick={()=>document.getElementById("coach-history")?.scrollIntoView({behavior:"smooth",block:"start"})}>STORICO</button></div></div>
   {error&&<div className="mt-4 rounded-xl border border-rose-500/20 bg-rose-500/5 p-3 text-[10px] text-rose-300">{error}</div>}
   {!athletes.length?<div className="mt-6 grid gap-3 md:grid-cols-[1.2fr_.8fr]"><div className="rounded-2xl border border-line bg-panel p-5"><div className="section-kicker">I TUOI ATLETI</div><h2 className="mt-2 text-2xl font-extrabold tracking-tight">Nessun atleta collegato.</h2><p className="mt-2 max-w-xl text-xs leading-5 text-zinc-500">Condividi il tuo codice Coach di 10 caratteri. Dopo il collegamento, i dati di allenamento appariranno qui automaticamente.</p><div className="mt-5 rounded-xl border border-line bg-panel2 p-4"><div className="field-label">PROSSIMO PASSO</div><div className="mt-2 text-sm font-bold">Attendi il primo collegamento atleta.</div><div className="mt-1 text-[9px] leading-4 text-zinc-600">Il Coach non deve creare gli atleti manualmente.</div></div></div><div className="rounded-2xl border border-accent bg-accent-soft p-5"><div className="section-kicker">COACHING LOOP</div><div className="mt-3 grid gap-2 text-[10px] font-bold"><div className="rounded-xl bg-panel2 p-3">01 · L’ATLETA SI ALLENA</div><div className="rounded-xl bg-panel2 p-3">02 · I DATI SI SINCRONIZZANO</div><div className="rounded-xl bg-panel2 p-3">03 · IL COACH REVISIONA</div><div className="rounded-xl bg-panel2 p-3">04 · IL COACH PUBBLICA</div></div></div></div>:<div className="mt-6 grid gap-3 md:grid-cols-[240px_1fr]">
    <div className="rounded-2xl border border-line bg-panel p-3"><div className="field-label">ATLETI</div><div className="mt-2 grid gap-1">{athletes.map(a=><button key={a.id} onClick={()=>{setSelected(a.id);setEditing(null)}} className={`rounded-xl border px-3 py-3 text-left ${selected===a.id?"border-accent-strong bg-accent-soft":"border-line bg-panel2"}`}><div className="text-xs font-bold">{a.display_name||"Athlete"}</div><div className="mt-1 text-[8px] text-zinc-600">{a.height_cm?`${a.height_cm} cm`:"Height —"} · {a.weight_kg?`${a.weight_kg} kg`:"Weight —"}</div></button>)}</div></div>
    <div className="min-w-0">
      ...
      <div id="coach-intelligence" className="mt-4 rounded-2xl border border-accent bg-accent-soft p-4"><div className="flex items-end justify-between gap-3"><div><div className="section-kicker">INTELLIGENZA COACH</div><div className="mt-1 text-[9px] text-zinc-500">Decision support from readiness, pain, recent volume and adherence. The Coach never changes the plan automatically.</div></div><span className="tag">REVISIONE MANUALE</span></div><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{decisions.map((d,i)=><div key={i} className="rounded-xl border border-line bg-panel p-3"><div className="flex items-start justify-between gap-2"><div className="text-[10px] font-extrabold">{d.title}</div><span className={`text-[8px] font-extrabold tracking-[.12em] ${d.level==="RED"?"text-rose-400":d.level==="YELLOW"?"text-amber-300":"text-emerald-400"}`}>{d.level}</span></div><div className="mt-2 text-[9px] leading-4 text-zinc-500">{d.detail}</div><div className="mt-2 text-[8px] font-bold tracking-[.12em] text-accent">{d.action}</div></div>)}</div></div>
      ...
      <div className="mt-5 rounded-2xl border border-line bg-panel p-4"><div className="flex items-end justify-between"><div><div className="section-kicker">REVISIONE PROGRESSOIONE</div><div className="mt-1 text-[9px] text-zinc-600">Dati dalle esposizioni reali. Il motore non promuove mai automaticamente.</div></div></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{insights.map((x:SkillInsight)=><div key={x.id} className="rounded-xl border border-line bg-panel2 p-3"><div className="flex items-start justify-between gap-3"><div><div className="text-[8px] font-bold tracking-[.12em] text-zinc-600">{x.skill} · {x.exposures} EXPOSURES</div><div className="mt-1 text-sm font-bold">{x.name}</div></div><span className="text-[8px] font-extrabold tracking-[.12em] text-accent">{x.decision}</span></div><div className="mt-2 text-[9px] text-zinc-500">{x.action} · {x.qualifyingStreak}/2 consecutive qualifying</div></div>)}{!insights.length&&<div className="text-xs text-zinc-600">Not enough logged data yet.</div>}</div></div>
      <div className="mt-4 flex gap-1 overflow-x-auto pb-1">{DAYS.map(d=><button key={d} onClick={()=>setDay(d)} className={`rounded-lg border px-3 py-2 text-[9px] font-bold whitespace-nowrap ${day===d?"border-accent-strong bg-accent-soft text-accent":"border-line bg-panel text-zinc-600"}`}>{d.slice(0,3).toUpperCase()}</button>)}</div>
...
function AuthScreen({onSignedIn}:{onSignedIn:(s:any)=>void}){
 ...
 return <div className="min-h-[100dvh] bg-ink text-white px-4 pt-[calc(env(safe-area-inset-top)+16px)]"><div className="mx-auto max-w-sm pt-16">
   <div className="eyebrow">CALISTHENICS <span className="text-accent">COACH</span></div><h1 className="mt-3 text-4xl font-extrabold tracking-tight">{title}</h1><p className="mt-3 text-xs leading-5 text-zinc-500">Un account per allenamenti, cronologia, progressi e sincronizzazione Coach.</p>
   <form onSubmit={submit} className="mt-8 rounded-2xl border border-line bg-panel p-4">
    {mode!=="reset"&&<div className="flex rounded-xl border border-line bg-panel2 p-1"><button type="button" className={`flex-1 rounded-lg py-2 text-[9px] font-bold ${mode==='sign_in'?'bg-accent-strong text-white':'text-zinc-600'}`} onClick={()=>{setMode('sign_in');setMsg('')}}>ACCEDI</button><button type="button" className={`flex-1 rounded-lg py-2 text-[9px] font-bold ${mode==='sign_up'?'bg-accent-strong text-white':'text-zinc-600'}`} onClick={()=>{setMode('sign_up');setMsg('')}}>CREA ACCOUNT</button></div>}
    ...
    {msg&&<div className="mt-3 rounded-xl border border-accent bg-accent-soft p-3 text-[9px] leading-4 text-zinc-400">{msg}</div>}
...
function Header({email,syncing,onSignOut}:{email?:string;syncing?:boolean;onSignOut?:()=>void}){
 return <header className="sticky top-0 z-30 border-b border-line bg-ink/90 pt-[env(safe-area-inset-top)] backdrop-blur-xl">
  <div className="mx-auto flex h-14 max-w-5xl items-center justify-between px-4 sm:h-16">
   <div className="text-[12px] font-extrabold tracking-[.14em]">CALISTHENICS <span className="text-accent">COACH</span></div>
...
function AthleteGoalCard(){
 ...
 return <div className="mt-4 rounded-2xl border border-accent bg-accent-soft p-4"><div className="section-kicker">FOCUS ATTUALE DEL COACH</div><div className="mt-2 text-sm font-extrabold">{profile.primaryGoal||"Sviluppo della performance"}</div><div className="mt-2 flex flex-wrap gap-2 text-[8px] font-bold tracking-[.1em] text-accent">{(profile.prioritySkills||[]).slice(0,4).map(x=><span key={x} className="chip">{x}</span>)}</div></div>
}
...
function CoachVerdict({session}:{session:SessionSummary}){
 const strong=scoreSessionSignals(session); const label=strong.status; const tone=label==="PROGRESS"?"text-emerald-300":label==="HOLD"?"text-accent":label==="RECOVERY"?"text-amber-300":"text-zinc-400";
 ...
 const dTone=(d:ExerciseCoachDecision["decision"])=>d==="PROGRESS"?"text-emerald-300":d==="REGRESS"?"text-rose-300":d==="REDUCE_VOLUME"?"text-amber-300":d==="REVIEW"?"text-orange-300":"text-accent";
...
function Today({day,setDay,start,draft,onResume}:{day:DayKey;setDay:(x:DayKey)=>void;start:()=>void;draft:any;onResume:()=>void}){
 ...
 return <div className="today-screen">
  ...
  <div className="today-context rounded-2xl border border-line bg-panel2 p-4">
   <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="section-kicker">FOCUS DI OGGI</div><div className="mt-1 text-lg font-extrabold tracking-tight">{p.title}</div><p className="today-focus-copy">{uiCopy(p.subtitle)}</p></div><span className="phase-pill">{phase}</span></div>
   <div className="today-focus-stats"><span>{p.blocks.length} ESERCIZI</span><span>{p.blocks.reduce((n,b)=>n+(b.sets||0),0)} SERIE</span><span>{push?"PUSH PRIORITY":"SKILL + PERFORMANCE"}</span></div>
  </div>
  <div className="mt-7"><div className="flex items-end justify-between gap-3"><div><div className="section-kicker">SESSIONE</div><p className="today-section-copy">Quello che farai, nell'ordine.</p></div><span className="session-total">{p.blocks.length} esercizi</span></div><div className="mt-3 grid gap-2">
   <div className="session-row session-row-warmup"><div className="session-index">00</div><div className="min-w-0"><span className="tag">WARM-UP</span><strong>{push?"Push / polsi / spalle":"Pull / gomiti / spalle"}</strong><p>{uiCopy(p.warmup.map(x=>x.name).join(" • "))}</p></div></div>
   {p.blocks.map((b,i)=><div key={b.id} className="session-row"><div className="session-index">{String(i+1).padStart(2,"0")}</div><div className="min-w-0"><div className="session-row-head"><div className="min-w-0"><span className="tag">{LABEL[b.kind]}</span><strong>{currentVariantFor(b.id,b.name)}</strong></div><span className="prescription-badge">{b.kind==="EMOM"?`${b.minutes||10} MIN`:`${b.sets||1} × ${b.target}`}</span></div><p className="session-row-detail">{displayBlockDetail(b,uiCopy)}</p>{coachNoteForBlock(b,uiCopy)&&<div className="session-coach-note"><span>COACH · </span>{coachNoteForBlock(b,uiCopy)}</div>}</div></div>)}
  </div></div>
  <AthleteGoalCard />
...
function TrendRow({name,values,unit}:{name:string;values:number[];unit:string}){
 ...
 return <div className="rounded-2xl border border-line bg-panel p-4"><div className="flex items-end justify-between gap-3"><div><span className="field-label">{name}</span><div className="mt-1 text-sm font-extrabold">{v.length?v[v.length-1].toFixed(unit==="s"?1:0):"—"}<span className="ml-1 text-[9px] text-zinc-600">{unit}</span></div></div><span className="text-[9px] text-zinc-600">ULTIMO {v.length}/6</span></div>{v.length>1&&<div className="mt-4 flex h-10 items-end gap-1">{v.map((x,i)=><div key={i} className="flex-1 rounded-sm bg-accent-muted" style={{height:`${Math.max(10,Math.round((x/max)*100))}%`}}/> )}</div>}{v.length>1&&<div className="mt-2 text-[8px] text-zinc-600">{v[0].toFixed(unit==="s"?1:0)} → {v[v.length-1].toFixed(unit==="s"?1:0)} {unit}</div>}</div>
}
...
function Progress({refresh}:{refresh:number}){
 ...
 return <div>
  ...
  <div className="mt-8"><div className="flex items-end justify-between"><div><div className="section-kicker">ARCHIVIO PR</div><p className="mt-1 text-[10px] text-zinc-600">Miglior performance registrata, non il target di oggi.</p></div></div>
    <div className="mt-3 grid gap-2 sm:grid-cols-2">{prs.map(pr=><button key={pr.id} className="rounded-2xl border border-line bg-panel p-4 text-left transition hover:border-accent-strong" onClick={()=>setSelected(pr.id)}>
  ...
  <div className="mt-8"><div className="section-kicker">PROSSIMI TRAGUARDI</div><div className="mt-3 grid gap-2 md:grid-cols-2">{milestones.map(m=><div key={m.id} className="rounded-2xl border border-accent bg-accent-soft p-4"><div className="flex items-start justify-between gap-3"><div><span className="field-label">{m.status}</span><div className="mt-1 text-sm font-extrabold">{m.name}</div></div><span className="text-[9px] font-bold text-accent">{m.progress}</span></div><p className="mt-3 text-[10px] leading-5 text-zinc-500">{m.detail}</p></div>)}{!milestones.length&&<div className="rounded-2xl border border-line bg-panel p-4 text-xs text-muted">Completa alcune sessioni per sbloccare i traguardi.</div>}</div></div>
...
function GoalProgressCard({goal,target,label}:{goal:"push-up"|"dips";target:number;label:string}){
 ...
 return <div className="rounded-2xl border border-line bg-panel p-4">
   <div className="flex items-start justify-between gap-3"><div><div className="field-label">{label}</div><div className="mt-1 text-2xl font-extrabold">{best||"—"} <span className="text-xs font-bold text-zinc-600">/ {target}</span></div></div><span className={`text-[8px] font-extrabold tracking-[.12em] ${status==="TARGET RAGGIUNTO"?"text-emerald-400":status==="PROGRESSING"?"text-accent":status==="CONTROLLA FATICA"?"text-amber-300":"text-zinc-500"}`}>{status}</span></div>
   <div className="mt-3 h-2 overflow-hidden rounded-full bg-zinc-900"><div className="h-full rounded-full bg-accent transition-all" style={{width:`${pct}%`}}/></div>
...
function CoachGoalsDashboard(){
 ...
 return <section className="mt-6 rounded-2xl border border-accent bg-accent-soft p-4">
   <div className="flex items-end justify-between gap-3">
...
function GoalAnalyticsCard({snapshot:g}:{snapshot:GoalPerformanceSnapshot}){
 ...
 const tone=status==='TARGET RAGGIUNTO'?'text-emerald-400':status==='PROGRESSING'?'text-accent':status==='CONTROLLA RECUPERO'?'text-amber-300':'text-zinc-500';
...
function Reports({refresh}:{refresh:number}){
 ...
 const weeklyTone=weekSessions.length>=4 && weekSessions.length <= 6 ? "SETTIMANA STABILE" : weekSessions.length > 6 ? "ALTO VOLUME" : "RACCOGLI DATI";
 const weeklySummary=weekSessions.length>=4 ? (best>0 ? "La settimana ha abbastanza esposizioni per leggere andamento e recupero." : "La settimana ha abbastanza esposizioni: continua a registrare gli esercizi chiave.") : "Servono ancora alcune sessioni per distinguere un segnale reale da una singola giornata.";
 return <div>
  <div className="eyebrow">REPORT</div><h1>Come sta andando?</h1>
  <section className="mt-5 rounded-2xl border border-line bg-panel p-4">
   <div className="flex items-start justify-between gap-3"><div><div className="section-kicker">STATO DELLA SETTIMANA</div><div className="mt-2 text-2xl font-extrabold">{weeklyTone}</div><p className="mt-2 text-[10px] leading-5 text-zinc-500">{weeklySummary}</p></div><span className="tag">{weekSessions.length}/6 SESSIONI</span></div>
   <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><Metric label="WORKOUT" value={weekSessions.length}/><Metric label="REPS" value={weekReps}/><Metric label="EMOM" value={weekEmom}/><Metric label="STATICO BEST" value={best?`${best.toFixed(1)}s`:"—"}/></div>
  </section>
  <CoachGoalsDashboard/>
  <div className="mt-6 rounded-2xl border border-line bg-panel p-4">
   <div className="flex items-end justify-between gap-3"><div><div className="section-kicker">RECENT COMPLETED WORKOUTS</div><p className="mt-1 text-[9px] text-zinc-600">Apri una sessione per rivedere esecuzione e report Coach.</p></div><span className="tag">ULTIMO {Math.min(7,sessions.length)}</span></div>
  ...
  <div className="mt-6 rounded-2xl border border-accent bg-accent-soft p-4"><div className="flex items-start justify-between gap-3"><div><div className="section-kicker">COSA FARE CON QUESTI DATI</div><div className="mt-1 text-lg font-extrabold">{weeklyIntelligence.headline}</div><p className="mt-1 text-[9px] leading-4 text-zinc-500">{weeklyIntelligence.summary}</p></div><span className="tag">{weeklyIntelligence.adherencePct.toFixed(0)}% ADHERENCE</span></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{weeklyIntelligence.actions.slice(0,4).map((a,i)=><div key={i} className="rounded-xl border border-line bg-panel p-3"><div className="text-[8px] font-extrabold tracking-[.1em] text-accent">{a.priority}</div><div className="mt-1 text-[10px] font-bold text-zinc-200">{a.title}</div><div className="mt-1 text-[9px] leading-4 text-zinc-600">{a.detail}</div></div>)}</div></div>
...
function AnalyticsPanel(){
 ...
 return <div className="mt-8 rounded-2xl border border-line bg-panel p-4">
  ...
  <div className="mt-4 flex flex-wrap items-center gap-2"><span className="field-label">PERIODO</span>{[7,30,90].map(d=><button key={d} className={`mini-btn min-w-12 ${days===d?'!border-accent-strong !text-white':''}`} onClick={()=>{setDays(d);setTimeout(()=>setIntel(analyticsIntelligence(d)),0)}}>{d}D</button>)}<button className="secondary-cta !py-2 !px-3" onClick={()=>{refresh();track("analytics_view_refreshed",{windowDays:days})}}>AGGIORNA</button></div>
...
function Settings(){return <div className="settings-page">
 <div className="eyebrow">{t("settings")}</div>
 <h1>{t("settingsTitle")}</h1>
 <p className="sub">{t("settingsSub")}</p>

 <section className="settings-section">
  <div className="settings-section-heading"><div><div className="section-kicker">ACCOUNT</div><p className="settings-section-copy">Accesso, sincronizzazione e collegamento con il Coach.</p></div></div>
  <CoachLinkCard onLinked={()=>track("coach_linked")}/>
 </section>

 <section className="settings-section">
  <div className="settings-section-heading"><div><div className="section-kicker">PROFILO ATLETA</div><p className="settings-section-copy">Obiettivi, contesto di coaching, attrezzatura e ritmo di allenamento.</p></div></div>
  <AthleteProfileEditor/>
 </section>

 <section className="settings-section">
  <div className="settings-section-heading"><div><div className="section-kicker">DATI</div><p className="settings-section-copy">Backup locale, importazione ed eventuale sincronizzazione cloud.</p></div></div>
  <DataBackup/>
 </section>

 <section className="settings-section settings-section-last">
  <div className="settings-section-heading"><div><div className="section-kicker">ANALISI UTILIZZO</div><p className="settings-section-copy">Metriche locali utili a individuare attriti nell'app.</p></div></div>
  <AnalyticsPanel/>
 </section>
 </div>}
function AthleteProfileEditor(){
 ...
 return <div className="mt-8 rounded-2xl border border-line bg-panel p-4">
  ...
  {profile.notes&&<div className="mt-3 rounded-xl border border-accent-subtle bg-accent-soft p-3"><span className="field-label">COACH CONTEXT</span><div className="mt-1 text-[9px] leading-4 text-zinc-400">{profile.notes}</div></div>}
...
function WorkoutPlayer({state,setState,refresh}:{state:any;setState:(x:any)=>void;refresh:()=>void}){
 ...
 return <div className="workout-player-shell fixed inset-0 z-50 flex min-h-[100dvh] flex-col bg-ink text-white">
  <div className="workout-player-header shrink-0 border-b border-line bg-ink/95 pt-[env(safe-area-inset-top)] backdrop-blur-xl">
   ...
   </div><div className="h-0.5 bg-zinc-900"><div className="h-full bg-accent transition-all" style={{width:`${progress}%`}}/></div>
  </div>
  ...
  {modifyAsk&&<div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm"><div className="w-full max-w-sm rounded-2xl border border-line bg-panel p-5"><div className="eyebrow">SOSTITUZIONE</div><h2 className="mt-2 text-xl font-extrabold">Cosa hai fatto al posto di questo esercizio?</h2><p className="mt-2 text-xs text-muted">L’esercizio originale verrà registrato come modificato. L’allenamento continuerà dopo il recupero di transizione.</p><input autoFocus className="mt-4 w-full" value={modification} onChange={e=>setModification(e.target.value)} placeholder="es. Dragon Flag" onKeyDown={e=>{if(e.key==="Enter")confirmModify()}}/><div className="mt-5 grid grid-cols-2 gap-2"><button className="secondary-cta" onClick={()=>{setModifyAsk(false);setModification("")}}>ANNULLA</button><button className="primary-cta" disabled={!modification.trim()} onClick={confirmModify}>SALVA SOSTITUZIONE</button></div></div></div>}
 </div>
}
...
function WarmupPlayer({steps,sound,vibration,onDone}:{steps:any[];sound:boolean;vibration:boolean;onDone:()=>void}){
 ...
 if(!steps.length)return <div className="flex flex-1 flex-col justify-center"><div className="eyebrow">WARM-UP</div><h2 className="mt-2 text-3xl font-extrabold">Nessun warm-up programmato.</h2><button className="primary-cta mt-6 w-full" onClick={onDone}>INIZIA PRIMO ESERCIZIO</button></div>;
 const next=()=>setI(v=>Math.min(steps.length-1,v+1));
 return <div className="flex flex-1 flex-col">
  <div className="eyebrow">WARM-UP</div><h2 className="mt-2 text-4xl font-extrabold">Prepara le articolazioni.</h2>
  <div className="mt-6 rounded-2xl border border-line bg-panel p-5"><div className="flex items-center justify-between"><div className="text-[9px] text-zinc-600">STEP {i+1}/{steps.length}</div><span className="text-[8px] font-bold tracking-[.12em] text-accent">PREPARAZIONE</span></div><strong className="mt-2 block">{s.name}</strong><p className="mt-1 text-[10px] text-muted">{s.dose}</p>{timer!==null&&<div className="mt-6 text-center text-5xl font-extrabold tracking-tight">{formatClock(timer)}</div>}</div>
  <div className="mt-6 grid grid-cols-2 gap-2 pb-[calc(env(safe-area-inset-bottom)+8px)] pt-4 sm:mt-auto sm:pt-8"><button className="secondary-cta" onClick={()=>setI(Math.max(0,i-1))} disabled={i===0}>INDIETRO</button>{i<steps.length-1?<button className="primary-cta" onClick={next}>PROSSIMO</button>:<button className="primary-cta" onClick={onDone}>INIZIA PRIMO ESERCIZIO</button>}</div>
...
function Rest({seconds,onSkip,onDone,sound=true,vibration=true,label="RECUPERO",nextLabel}:{seconds:number;onSkip:()=>void;onDone:()=>void;sound?:boolean;vibration?:boolean;label?:string;nextLabel?:string}){
 ...
 return <div className="rest-player flex flex-1 flex-col items-center justify-center py-8">
   <div className="text-center"><div className="field-label">{label}</div>{nextLabel&&<div className="mt-2 text-sm font-bold text-zinc-300">PROSSIMO · {nextLabel}</div>}</div>
   <div className="relative mt-8 h-[232px] w-[232px]">
     <svg className="h-full w-full -rotate-90" viewBox="0 0 232 232" aria-hidden="true"><circle cx="116" cy="116" r={radius} fill="none" stroke="currentColor" strokeWidth="8" className="text-zinc-900"/><circle cx="116" cy="116" r={radius} fill="none" stroke="currentColor" strokeWidth="8" strokeLinecap="round" className="text-accent-soft transition-[stroke-dashoffset] duration-200" strokeDasharray={circ} strokeDashoffset={circ*(1-progress)}/></svg>
...
function SetBlock({block,day,onComplete,onStarted,existing,onProgress,sound,vibration}:{block:ExerciseBlock;day:DayKey;onComplete:(l:WorkoutLog)=>void;onStarted:()=>void;existing?:WorkoutLog;onProgress:(l:WorkoutLog)=>void;sound:boolean;vibration:boolean}){
 ...
 const startedFeedbackRef=useRef(false);
 useEffect(()=>{if(startedFeedbackRef.current)return;startedFeedbackRef.current=true;feedback("start",sound,vibration)},[sound,vibration]);
 ...
 const saveSet=()=>{const n=Number(input);if(!Number.isFinite(n)||n<0||complete||rest)return;const next=[...reps,n];setReps(next);setInput("");const shouldRest=shouldRestAfterStandardSet(next.length,max);setRest(shouldRest);feedback("complete",sound,vibration);onStarted();persist(next)};
 ...
 return <div className="set-player flex flex-1 flex-col pt-5"><div className="set-player-target"><TargetPanel block={block} target={target} onChange={setTarget}/><div className="mt-6 text-center"><div className="text-[9px] tracking-[.15em] text-zinc-600">SET {Math.min(reps.length+1,max)} / {max}</div><span className="field-label mt-3">REPS DELLA SERIE</span><div className="mt-2 flex items-center justify-center gap-2"><button type="button" aria-label="Diminuisci ripetizioni" title="Diminuisci" className="counter-btn" disabled={complete} onClick={()=>setInput(String(Math.max(0,(Number(input)||0)-1)))}><Minus size={16}/></button><input aria-label="Reps completed this set" className="counter-input" inputMode="numeric" value={input} onChange={e=>setInput(e.target.value.replace(/\D/g,""))} placeholder="0" disabled={complete}/><button type="button" aria-label="Aumenta ripetizioni" title="Aumenta" className="counter-btn" disabled={complete} onClick={()=>setInput(String((Number(input)||0)+1))}><Plus size={16}/></button></div>{last!==undefined&&<div className="mt-3 text-[10px] text-zinc-500">ULTIMA SERIE · {last} reps{diff!==null&&<span className={`ml-2 ${diff>=0?"text-emerald-400":"text-rose-300"}`}>{diff>=0?`+${diff}`:`${diff}`}</span>}</div>}</div></div><div className="set-player-actions mt-6 grid gap-2 sm:grid-cols-2">{!complete&&!rest&&<button className="primary-cta sm:col-span-2" onClick={saveSet}>SALVA SERIE</button>}{complete&&!rest&&<button className="primary-cta sm:col-span-2" onClick={()=>finish("complete")}>PROSSIMO ESERCIZIO</button>}{complete&&rest&&<button className="secondary-cta sm:col-span-2" disabled>RECUPERO PRIMA DEL PROSSIMO ESERCIZIO</button>}{reps.length>0&&!complete&&<button className="secondary-cta sm:col-span-2" onClick={()=>finish("incomplete")}>SALVA INCOMPLETO</button>}</div>{rest&&<Rest seconds={block.rest} sound={sound} vibration={vibration} onSkip={()=>{feedback("start",sound,vibration);setRest(false)}} onDone={()=>setRest(false)}/>}<div className="mt-4 flex flex-wrap justify-center gap-2">{reps.map((r,i)=><button className="chip" key={i} title="Tocca per modificare questa serie" onClick={()=>{setEditing(i);setEditValue(String(r))}}>SERIE {i+1}: {r}</button>)}</div>{editing!==null&&<div className="mt-3 rounded-xl border border-line bg-panel p-3"><span className="field-label">MODIFICA SERIE {editing+1}</span><div className="mt-1 flex items-center gap-2"><button className="counter-btn" onClick={()=>setEditValue(String(Math.max(0,Number(editValue||0)-1)))}><Minus size={16}/></button><input className="counter-input flex-1" inputMode="numeric" value={editValue} onChange={e=>setEditValue(e.target.value.replace(/\D/g,""))}/><button className="counter-btn" onClick={()=>setEditValue(String(Number(editValue||0)+1))}><Plus size={16}/></button></div><div className="mt-2 grid grid-cols-2 gap-2"><button className="secondary-cta" onClick={()=>setEditing(null)}>ANNULLA</button><button className="primary-cta" onClick={()=>{const n=Number(editValue);if(Number.isFinite(n)&&n>=0){const next=reps.map((x,j)=>j===editing?n:x);setReps(next);persist(next)}setEditing(null)}}>SALVA MODIFICA</button></div></div>}<div className="mt-auto pb-4 pt-8">{block.bandOptions&&<label className="block mb-3"><span className="field-label">LOOP</span><select value={band} onChange={e=>setBand(e.target.value as Band)}>{block.bandOptions.map(b=><option key={b}>{b}</option>)}</select></label>}<EffortPicker rir={rir} setRir={setRir} fatigue={fatigue} setFatigue={setFatigue}/></div></div>
}

function EffortPicker({rir,setRir,fatigue,setFatigue}:{rir:string;setRir:(v:string)=>void;fatigue:string;setFatigue:(v:string)=>void}){return <div className="effort-panel mt-4 rounded-2xl border border-line bg-panel p-3"><div className="grid gap-4 sm:grid-cols-2"><div><div className="flex items-center justify-between"><span className="field-label">QUANTE REPS AVEVI ANCORA?</span>{rir!==''&&<span className="text-[8px] font-bold text-accent">{rirLabel(Number(rir))}</span>}</div><div className="mt-2 grid grid-cols-4 gap-2">{RIR_OPTIONS.map(v=><button type="button" key={v} onClick={()=>setRir(rir===String(v)?'':String(v))} aria-pressed={rir===String(v)} className={`rounded-xl border px-2 py-3 text-[10px] font-extrabold transition ${rir===String(v)?'effort-option-active':'effort-option'}`}>{v===3?'3+':v}</button>)}</div></div><div><div className="flex items-center justify-between"><span className="field-label">QUANTO È STATA DURA?</span>{fatigue!==''&&<span className="text-[8px] font-bold text-accent">{fatigueLabel(Number(fatigue))}</span>}</div><div className="mt-2 grid grid-cols-5 gap-1.5">{FATIGUE_OPTIONS.map(v=><button type="button" key={v} onClick={()=>setFatigue(fatigue===String(v)?'':String(v))} aria-pressed={fatigue===String(v)} className={`rounded-xl border px-1 py-3 text-[10px] font-extrabold transition ${fatigue===String(v)?'effort-option-active':'effort-option'}`}>{v}</button>)}</div><div className="mt-1 text-right text-[8px] text-zinc-600">1 facile · 5 quasi esausto</div></div></div></div>}
...
function SideSetBlock({block,day,onComplete,onStarted,existing,onProgress,sound,vibration}:{block:ExerciseBlock;day:DayKey;onComplete:(l:WorkoutLog)=>void;onStarted:()=>void;existing?:WorkoutLog;onProgress:(l:WorkoutLog)=>void;sound:boolean;vibration:boolean}){
 ...
 const save=()=>{const n=Number(input);if(!Number.isFinite(n)||n<0||complete||rest)return;const savedSide=side,nextReps=[...reps,n],nextSides=[...sides,savedSide],isLastSide=nextReps.length>=totalSides;setReps(nextReps);setSides(nextSides);setInput("");const shouldRest=shouldRestAfterSideSet(nextReps.length,totalSides,savedSide);feedback("complete",sound,vibration);onStarted();persist(nextReps,nextSides);const nextSide=savedSide==="R"?"L":"R";if(!shouldRest&&!isLastSide)feedback("start",sound,vibration);setSide(nextSide);setRest(shouldRest)};
 ...
 const r=range(block.target);
 return <div className="side-set-player flex flex-1 flex-col pt-5"><div className="rounded-2xl border border-accent bg-accent-soft p-4"><div className="flex items-end justify-between gap-4"><div><span className="field-label">LATO ATTIVO</span><div className="mt-1 text-3xl font-extrabold tracking-tight">{side==="R"?"DESTRA":"SINISTRA"}</div><div className="mt-1 text-[10px] text-muted">SERIE {Math.min(round,totalRounds)} / {totalRounds}</div></div><div className="text-right"><span className="field-label">TARGET / LATO</span><div className="mt-1 text-base font-extrabold tabular-nums">{r.min===r.max?r.min:`${r.min}–${r.max}`} reps</div></div></div></div><div className="mt-6 text-center"><div className="text-[9px] tracking-[.15em] text-zinc-600">LATO {Math.min(reps.length+1,totalSides)} / {totalSides}</div><span className="field-label mt-3">REPS DELLA SERIE</span><div className="mt-2 flex items-center justify-center gap-2"><button type="button" aria-label="Diminuisci ripetizioni" title="Diminuisci" className="counter-btn" disabled={complete} onClick={()=>setInput(String(Math.max(0,(Number(input)||0)-1)))}><Minus size={16}/></button><input aria-label={`Reps completed ${side==="R"?"right":"left"}`} className="counter-input" inputMode="numeric" value={input} onChange={e=>setInput(e.target.value.replace(/\D/g,""))} placeholder="0" disabled={complete}/><button type="button" aria-label="Aumenta ripetizioni" title="Aumenta" className="counter-btn" disabled={complete} onClick={()=>setInput(String((Number(input)||0)+1))}><Plus size={16}/></button></div>{pr[currentIndex]!==undefined&&<div className="mt-3 text-[10px] text-zinc-500">ULTIMO · {ps[currentIndex]==="R"?"DESTRA":"SINISTRA"}: <b className="text-white">{pr[currentIndex]}</b></div>}</div>{!complete&&!rest&&<button className="primary-cta mt-6" onClick={save}>SALVA {side==="R"?"DESTRA":"SINISTRA"}</button>}{complete&&!rest&&<button className="primary-cta mt-2" onClick={()=>finish("complete")}>PROSSIMO ESERCIZIO</button>}{complete&&rest&&<button className="secondary-cta mt-2" disabled>RECUPERO PRIMA DEL PROSSIMO ESERCIZIO</button>}{reps.length>0&&!complete&&<button className="secondary-cta mt-2" onClick={()=>finish("incomplete")}>SALVA INCOMPLETO</button>}{rest&&<Rest seconds={block.rest} sound={sound} vibration={vibration} onSkip={()=>setRest(false)} onDone={()=>setRest(false)}/>}<div className="mt-5 flex flex-wrap justify-center gap-2">{reps.map((r,i)=><button className="chip" key={i} onClick={()=>{setReps(v=>v.filter((_,j)=>j!==i));setSides(v=>v.filter((_,j)=>j!==i))}}>{sides[i]} {i+1}: {r}</button>)}</div><div className="mt-auto pb-4 pt-8">{block.bandOptions&&<label className="block mb-3"><span className="field-label">LOOP</span><select value={band} onChange={e=>setBand(e.target.value as Band)}>{block.bandOptions.map(b=><option key={b}>{b}</option>)}</select></label>}<EffortPicker rir={rir} setRir={setRir} fatigue={fatigue} setFatigue={setFatigue}/></div></div>
}
...
function StaticSkill({block,day,onComplete,onStarted,sound,vibration,existing,onProgress}:{block:ExerciseBlock;day:DayKey;onComplete:(l:WorkoutLog)=>void;onStarted:()=>void;sound:boolean;vibration:boolean;existing?:WorkoutLog;onProgress:(l:WorkoutLog)=>void}){
 ...
 const stop=()=>{if(stopLock.current||phase!=="hold")return;stopLock.current=true;const sec=Math.min(r.max,Number(elapsed.toFixed(1)));const next=[...vals,sec],nextQ=attemptQuality?[...qualities,attemptQuality]:[...qualities];setVals(next);setQualities(nextQ);setAttemptQuality(undefined);setPhase("stopped");feedback("complete",sound,vibration);onProgress({id:existing?.id||crypto.randomUUID(),sessionId:existing?.sessionId||`draft:${day}`,date:Date.now(),day,exerciseId:block.id,exerciseName:block.name,variantId:getVariant(block.id)?.variantId||block.id,variantName,kind:block.kind,status:"incomplete",prescription:prescriptionSnapshot(block),result:{seconds:next,band,rir:rir?Number(rir):undefined,fatigue:fatigue?Number(fatigue):undefined,quality:nextQ as ("Clean"|"Shaky"|"Lost position")[],note:`Coach range ${block.target}`}})};
 ...
 return <div className="static-skill-player flex flex-1 flex-col items-center pt-2">
   ...
   {vals.length>0&&<div className="mt-5 w-full max-w-sm rounded-xl border border-line bg-panel2 p-3 text-center"><div className="flex flex-wrap justify-center gap-2">{vals.map((v,i)=><span key={i} className="chip">{v.toFixed(1)}s</span>)}</div>{maxed&&allClean&&<div className="mt-3 text-[9px] font-bold text-emerald-400">PROGRESSOIONE PRONTA</div>}{allInRange&&!maxed&&allClean&&<div className="mt-3 text-[9px] font-bold text-accent">TARGET COMPLETATO</div>}</div>}
...
function CoachProposalPanel({session}:{session:SessionSummary}){
 ...
 return <section className="mt-4 rounded-2xl border border-accent bg-accent-soft p-5"><div className="flex items-start justify-between gap-3"><div><div className="section-kicker">PROPOSTE DEL COACH</div><p className="mt-1 text-[10px] leading-4 text-zinc-500">Approva o rifiuta la proposta. Il workout resta invariato finché non la accetti.</p></div><span className="tag">APPROVAZIONE UMANA</span></div>{message&&<div className="mt-3 rounded-lg bg-emerald-500/10 p-2 text-[9px] text-emerald-300">{message}</div>}<div className="mt-4 space-y-2">{items.map(p=><div key={p.id} className="rounded-xl border border-line bg-panel p-3"><div className="flex items-start justify-between gap-3"><div><div className="text-[8px] font-bold tracking-[.12em] text-accent">{p.type.toUpperCase()}</div><div className="mt-1 text-sm font-extrabold">{p.title}</div></div><span className="text-[8px] font-bold text-zinc-600">{p.status.toUpperCase()}</span></div><div className="mt-3 grid grid-cols-2 gap-2"><div><div className="field-label">ATTUALE</div><div className="mt-1 text-sm font-bold">{p.from}</div></div><div><div className="field-label">PROPOSTA</div><div className="mt-1 text-sm font-bold text-accent">{p.to}</div></div></div><p className="mt-3 text-[9px] leading-4 text-zinc-500">{p.reason}</p>{p.status==="pending"&&<div className="mt-3 grid grid-cols-2 gap-2"><button className="secondary-cta" onClick={()=>void apply(p,false)}>RIFIUTA</button><button className="primary-cta" onClick={()=>void apply(p,true)}>ACCETTA E AGGIORNA IL PIANO</button></div>}{p.status!=="pending"&&<div className="mt-3 text-[9px] font-bold text-zinc-500">DECISIONE REGISTRATA: {p.status.toUpperCase()}</div>}</div>)}</div></section>;
}
 ...
 return <div className="fixed inset-0 z-[60] flex min-h-[100dvh] flex-col bg-ink text-white">
   ...
   <div className="min-h-0 flex-1 overflow-y-auto"><div className="mx-auto flex min-h-full max-w-2xl flex-col px-4 py-8 pb-28">
     ...
     <div className="mt-8 text-center">
       ...
       <div className="mt-2 text-[9px] font-bold tracking-[.15em] text-accent">{isStatic?"MANTIENI · RESPIRA · RILASSA":"MUOVITI CON CONTROLLO"}</div>
     </div>
